...
\add{To check that our results were robust to the choice of phylogenetic
method, we constructed phylogenies for two of the datasets (mixed/Spain and
IDU/Estonia) with BEAST~\autocite{drummond2007beast}. We used the TN93 model of
sequence evolution~\autocite{tamura1993estimation}, a strict molecular clock,
and constant population size. The chains were run for $10^7$ steps and sampled
every $10^3$ steps. Maximum clade credibility (MCC) trees were constructed from
the sampled trees after discarding the first 10\% as burn-in.}
...
\begin{figure}[ht]
    \centering
    \includegraphics[width=0.6\textwidth]{{\Sexpr{file.path("abc-posterior", middle.plot)}}.pdf}
    \caption[
...
<<marginal, include=FALSE>>=
    ...
    d <- setDT(collect.data("../../simulations/abc-better-smc/point-estimate/*"))

    base.alpha <- d[nparticle == 1000, abs(alpha - true_alpha)]
    better.alpha <- d[nparticle == 2000, abs(alpha - true_alpha)]

    base.I <- d[nparticle == 1000, abs(I - true_I)]
    better.I <- d[nparticle == 2000, abs(I - true_I)]

    base.N <- d[nparticle == 1000, abs(N - true_N)]
    better.N <- d[nparticle == 2000, abs(N - true_N)]
@
...
when all other parameters were fixed. \Cref{fig:moresmc} shows the impact of
...
higher SMC settings were slightly worse (by
    \Sexpr{ round((better.alpha - base.alpha) * 100 / better.alpha) }\%,
    \Sexpr{ round((better.I - base.I) * 100 / better.I) }\%, and
    \Sexpr{ round((better.N - base.N) * 100 / better.N) }\%
for $\alpha$, $I$, and $N$ respectively). However, the 50\% HPD interval for
$I$ was slightly closer to the true value of 2000 with the improved settings
    (\Sexpr{ d[nparticle == 2000, round(I.lower.50)] } - 
     \Sexpr{ d[nparticle == 2000, round(I.upper.50)] }, vs.
     \Sexpr{ d[nparticle == 1000, round(I.lower.50)] } - 
     \Sexpr{ d[nparticle == 1000, round(I.upper.50)] } with basic settings).
