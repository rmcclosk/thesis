...
\Sexpr{nltt.alpha[,round(min(rsquared), 2)]}
and
  \Sexpr{nltt.alpha[,round(max(rsquared), 2)]}.
The $R^2$ of the linear regression against Sackin's index varied between
  \Sexpr{sackin.alpha[,round(min(rsquared), 2)]}
and
  \Sexpr{sackin.alpha[,round(max(rsquared), 2)]}.
...
especially on trees with a higher number of tips, with $R^2$ between
  \Sexpr{sackin.m[,round(min(rsquared), 2)]}
and
  \Sexpr{sackin.m[,round(max(rsquared), 2)]}.
...
500-tip transmission trees ($R^2$ = 
  \Sexpr{nltt.N[which(I == 2000 & ntip == 100), round(mean(rsquared), 2)]}
and
  \Sexpr{nltt.N[which(I == 2000 & ntip == 500), round(mean(rsquared), 2)]}),
although it performed poorly when $I$ was 500 or 1000 (average $R^2$ =
  \Sexpr{nltt.N[which(I < 2000), round(mean(rsquared), 2)]}).
Sackin's index could not classify $N$, with all $R^2$ below
  \Sexpr{sackin.N[,round(max(rsquared) + 0.01, 2)]}.
...
No other parameters influenced the accuracy of the $N$ estimates 
    (one-way ANOVA, $p \geq$ \Sexpr{min(N.av[1:3,"Pr(>F)"])}).
...
$\alpha$, 
  \Sexpr{d["alpha", map]} [\Sexpr{d["alpha", hpd.min]}-
                           \Sexpr{d["alpha", hpd.max]}];
$I$,
  \Sexpr{d["I", map]} [\Sexpr{d["I", hpd.min]}-
                       \Sexpr{d["I", hpd.max]}];
$m$,
  \Sexpr{d["m", floor(map)]} [\Sexpr{d["m", floor(hpd.min)]}-
                              \Sexpr{d["m", floor(hpd.max)]}];
$N$,
  \Sexpr{d["N", map]} [\Sexpr{d["N", hpd.min]}-
                       \Sexpr{d["N", hpd.max]}].

<<peerdriven, include=FALSE>>=
  d <- fread("bzcat ../../simulations/abc-pa-peerdriven/abc/*")
  d <- d[iter == max(iter)]
  d <- d[sample(1:nrow(d), prob=weight, replace=TRUE)]
  params <- c("alpha", "I", "m", "N")
  setnames(d, c("attach_power", "edges_per_vertex", "nodes", "sim_nodes"),
...
$\alpha$ values near unity (\gls{MAP} estimate [95\% \gls{HPD}] 
  \Sexpr{map["niculescu2015", alpha]} 
  [\Sexpr{low["niculescu2015", alpha]}-\Sexpr{up["niculescu2015", alpha]}]
and
  \Sexpr{map["wang2015", alpha]} 
  [\Sexpr{low["wang2015", alpha]}-\Sexpr{up["wang2015", alpha]}]). Another two
datasets~\autocite{li2015hiv, cuevas2009hiv} had lower estimated values and wider
\gls{HPD} intervals
  (\Sexpr{map["li2015", alpha]} 
  [\Sexpr{low["li2015", alpha]}-\Sexpr{up["li2015", alpha]}]
and
  \Sexpr{map["cuevas2009", alpha]} 
  [\Sexpr{low["cuevas2009", alpha]}-\Sexpr{up["cuevas2009", alpha]}]). 
The \textcite{novitsky2014impact} data had an extremely low estimated $\alpha$
and a very wide \gls{HPD} interval
  \Sexpr{map["novitsky2014", alpha]} 
  [\Sexpr{low["novitsky2014", alpha]}-\Sexpr{up["novitsky2014", alpha]}]). 
...
datasets
  (\textcite{cuevas2009hiv}, \Sexpr{map["cuevas2009", I]} 
  [\Sexpr{low["cuevas2009", I]}-\Sexpr{up["cuevas2009", I]}];
   \textcite{niculescu2015recent}, \Sexpr{map["niculescu2015", I]}
  [\Sexpr{low["niculescu2015", I]}-\Sexpr{up["niculescu2015", I]}])
and wider intervals around the other two
  (\textcite{li2015hiv}, \Sexpr{map["li2015", I]} 
  [\Sexpr{low["li2015", I]}-\Sexpr{up["li2015", I]}];
   \textcite{wang2015targeting}, \Sexpr{map["wang2015", I]}
  [\Sexpr{low["wang2015", I]}-\Sexpr{up["wang2015", I]}]).
...
estimated $I$, and \gls{HPD} interval spanning almost the entire prior region
  (\Sexpr{map["novitsky2014", I]} 
  [\Sexpr{low["novitsky2014", I]}-\Sexpr{up["novitsky2014", I]}]).
