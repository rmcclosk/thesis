...
\begin{table}
  ...
  \begin{tabular}{ccccc}
    ...
    \autocite{novitsky2013phylogenetic} & \multirow{2}{*}{201^*} &
  ...
  Acronyms: MSM, men who have sex with men. $^*$ Sub-sampled from 1299.}
  \label{tab:data}
\end{table}
...
<<classifiers, include=FALSE>>=
    get.kernel.data <- function (param) {
        md <- collect.metadata(paste0('../../simulations/kernel-', param, '/classifier/*'))
    ...
    kernel.alpha <- get.kernel.data('alpha')
    kernel.m <- get.kernel.data('m')
    kernel.I <- get.kernel.data('I')
    kernel.N <- get.kernel.data('N')
@
...
had an $R^2$ above 
    \Sexpr{kernel.alpha[,min(rsquared %/% 0.05 * 0.05)]} 
...
Across replicates and other model parameters, the mean absolute difference
between the estimated and true values of $\alpha$ was X, with a standard error
of X. 
