...
<<classifiers, include=FALSE>>=
    ...
    sackin.N <- get.kernel.data('N.bk.2', 'stats-classifier')
    ...
    nltt.N <- get.kernel.data('N.bk.2', 'nltt-classifier')
@
...
in any epidemic scenario, with average accuracy values of 
  \Sexpr{kernel.m[,round(mean(accuracy), 2)]} for kSVM,
  \Sexpr{nltt.m[,round(mean(accuracy), 2)]} for the nLTT, and
  \Sexpr{sackin.m[,round(mean(accuracy), 2)]}
...
\begin{figure}[ht]
    \centering
    \includegraphics[width=0.7\textwidth]{{\Sexpr{file.path("abc-posterior", middle.plot)}}.pdf}
    \caption[
...
<<realdata, include=FALSE>>=
  ...
  d1 <- fread(rownames(m)[m$m_min == 1 & m$agg_gene == "gag"])[dataset != "bctree"]
  d2 <- fread(rownames(m)[m$m_min == 2 & m$agg_gene == "gag"])[dataset != "bctree"]

  d1.env <- fread(rownames(m)[m$m_min == 1 & m$agg_gene == "env"])[dataset != "bctree"]
  d2.env <- fread(rownames(m)[m$m_min == 2 & m$agg_gene == "env"])[dataset != "bctree"]
  ...
  stopifnot(pmean1[dataset != "novitsky2014", dataset[which.max(I)]] == "li2015")
...
<<raxml, include=FALSE>>=
    d.raxml <- fread(Sys.glob("../../simulations/aggregates/raxml-hpd/*"))
@
...
(IDU/Estonia). When aggregated by risk group, the average estimates were
    \Sexpr{pmean1[c("zetterberg2004", "niculescu2015"), round(mean(alpha), 2)]}
for IDU,
...
<<genetrees, include=FALSE>>=
    datasets <- c("grabowski2014", "mccormack2002")
    ...
    ratios <- data.table(dataset=rep(datasets, each=2),
                         gene=rep(c("env", "gag"), 2),
                         ratio=sapply(trees, get.ratio))
    setkey(ratios, dataset)
    stopifnot(pmean1[datasets, round(mean(I))] < pmean1.env[datasets, round(mean(I))])
@
...
estimates from the \textit{env} data were higher (HET/Uganda,
    \Sexpr{pmean1["grabowski2014", round(I)]} for \textit{gag} vs.
    \Sexpr{pmean1.env["grabowski2014", round(I)]} for \textit{env};
HET/Malawi,
    \Sexpr{pmean1["mccormack2002", round(I)]} for \textit{gag} vs.
    \Sexpr{pmean1.env["mccormack2002", round(I)]} for \textit{env}).}
...
above
    \Sexpr{pmean2[c("zetterberg2004", "niculescu2015"), round(min(alpha), 2)]}.
...
<<bootstrap, include=FALSE>>=
    d.boot <- fread(Sys.glob("../../simulations/aggregates/bootstrap-hpd/*"))
@
...
means 
    \Sexpr{pmean2["cuevas2009", round(alpha, 2)]}
vs. 
    \Sexpr{d.raxml[dataset == "cuevas2009" & parameter == "alpha", round(mean, 2)]}
for mixed/Spain and 
    \Sexpr{pmean2["zetterberg2004", round(alpha, 2)]}
vs.
    \Sexpr{d.raxml[dataset == "zetterberg2004" & parameter == "alpha", round(mean, 2)]}
...
RAxML was used instead of FastTree to reconstruct the trees 
    (\Sexpr{pmean2["zetterberg2004", round(I)]} 
     vs. \Sexpr{d.raxml[dataset == "zetterberg2004" & parameter == "I", round(mean)]} for IDU/Estonia, 
    \Sexpr{pmean2["cuevas2009", round(I)]}
    vs. \Sexpr{d.raxml[dataset == "cuevas2009" & parameter == "I", round(mean)]} for mixed/Spain). 
...
estimated posterior mean [range of bootstrap posterior means] was
    \Sexpr{pmean2["cuevas2009", round(alpha, 2)]}
    [\Sexpr{d.boot[dataset == "cuevas2009" & parameter == "alpha" & replicate < 5, round(min(mean), 2)]} -
     \Sexpr{d.boot[dataset == "cuevas2009" & parameter == "alpha" & replicate < 5, round(max(mean), 2)]}]
for $\alpha$, 
    \Sexpr{pmean2["cuevas2009", round(I)]}
    [\Sexpr{d.boot[dataset == "cuevas2009" & parameter == "I" & replicate < 5, round(min(mean))]} -
     \Sexpr{d.boot[dataset == "cuevas2009" & parameter == "I" & replicate < 5, round(max(mean))]}] for $I$,
    \Sexpr{pmean2["cuevas2009", round(m, 2)]}
    [\Sexpr{d.boot[dataset == "cuevas2009" & parameter == "m" & replicate < 5, round(min(mean), 2)]} -
     \Sexpr{d.boot[dataset == "cuevas2009" & parameter == "m" & replicate < 5, round(max(mean), 2)]}] for $m$, and
    \Sexpr{pmean2["cuevas2009", round(N)]}
    [\Sexpr{d.boot[dataset == "cuevas2009" & parameter == "N" & replicate < 5, round(min(mean))]} -
     \Sexpr{d.boot[dataset == "cuevas2009" & parameter == "N" & replicate < 5, round(max(mean))]}] for $N$. 
For the IDU/Estonia data, values were
    \Sexpr{pmean2["zetterberg2004", round(alpha, 2)]}
    [\Sexpr{d.boot[dataset == "zetterberg2004" & parameter == "alpha" & replicate < 5, round(min(mean), 2)]} -
     \Sexpr{d.boot[dataset == "zetterberg2004" & parameter == "alpha" & replicate < 5, round(max(mean), 2)]}]
for $\alpha$, 
    \Sexpr{pmean2["zetterberg2004", round(I)]}
    [\Sexpr{d.boot[dataset == "zetterberg2004" & parameter == "I" & replicate < 5, round(min(mean))]} -
     \Sexpr{d.boot[dataset == "zetterberg2004" & parameter == "I" & replicate < 5, round(max(mean))]}] for $I$,
    \Sexpr{pmean2["zetterberg2004", round(m, 2)]}
    [\Sexpr{d.boot[dataset == "zetterberg2004" & parameter == "m" & replicate < 5, round(min(mean), 2)]} -
     \Sexpr{d.boot[dataset == "zetterberg2004" & parameter == "m" & replicate < 5, round(max(mean), 2)]}] for $m$, and
    \Sexpr{pmean2["zetterberg2004", round(N)]}
    [\Sexpr{d.boot[dataset == "zetterberg2004" & parameter == "N" & replicate < 5, round(min(mean))]} -
    \Sexpr{d.boot[dataset == "zetterberg2004" & parameter == "N" & replicate < 5, round(max(mean))]}] for $N$.}
...
<<realdata_I, include=FALSE>>=
    stopifnot(d1[parameter == "I", dataset[which.min(mean)]] == "zetterberg2004")
    stopifnot(d1[parameter == "I", dataset[which.max(mean)]] == "novitsky2014")
@
...
datasets ranged from
    \Sexpr{d1[parameter == "I", round(min(mean))]}
    (IDU/Estonia)
to
    \Sexpr{d1[parameter == "I", round(max(mean))]}
(HET/Botswana).}
