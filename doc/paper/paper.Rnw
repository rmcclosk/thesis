...
sequences were removed with BioPython.} Phylogenies were constructed from the
nucleotide alignments by approximate maximum likelihood using
...
<<classifiers, include=FALSE>>=
    get.kernel.data <- function (param, step) {
        ...
        if (param == 'alpha') {
            d[,list(rsquared=mean(rsquared), accuracy=mean(accuracy)), by=c('nsimnode', 'ntip')]
        } else if (param == 'm') {
            d[,list(rsquared=mean(rsquared)), by=c('nsimnode', 'ntip')]
        } else if (param == 'I') {
            d[,list(rsquared=mean(rsquared), accuracy=mean(accuracy)), by=ntip]
        } else {
...
resulted in low accuracy in some cases (\textit{e.g.} \cref{fig:alphacrossvn},
center row).}
...
The kSVM classifier for $\alpha$ had an average accuracy of 
    \Sexpr{kernel.alpha[,round(mean(accuracy), 2)]},
compared to 
    \Sexpr{nltt.alpha[,round(mean(accuracy), 2)]}
for the nLTT, and
    \Sexpr{sackin.alpha[,round(mean(accuracy), 2)]}
...
in any epidemic scenario, with average $R^2$ values of 
...
tips in the tree. For 100-tip trees, the average accuracy was
  \Sexpr{kernel.I[ntip == 100, round(mean(accuracy), 2)]},
  \Sexpr{nltt.I[ntip == 100, round(mean(accuracy), 2)]}, and
  \Sexpr{sackin.I[ntip == 100, round(mean(accuracy), 2)]}
for the tree kernel, nLTT, and Sackin's index respectively.
For 500-tip
trees, the values increased to
  \Sexpr{kernel.I[ntip == 500, round(mean(accuracy), 2)]},
  \Sexpr{nltt.I[ntip == 500, round(mean(accuracy), 2)]}, and
  \Sexpr{sackin.I[ntip == 500, round(mean(accuracy), 2)]}.
...
<<glm_table, echo=FALSE, warning=FALSE, error=FALSE, results="asis">>=
    format.glm <- function (df) {
        df.copy <- copy(df)
        df.copy[Parameter == "alpha", Parameter := "\\alpha"]
        df.copy[Parameter != "(Intercept)", Parameter := paste0("$", Parameter, "$")]
    ...
    glm.tbl <- rbind(format.glm(alpha.glm), format.glm(I.glm))
    glm.tbl <- cbind(`Dependent variable`=c("$\\alpha$", "", "", "", "$I$", "", "", ""),
                     glm.tbl)
    setnames(glm.tbl, "Parameter", "Independent variable")

    capt.short <- paste("Parameters of fitted GLMs relating error in estimated",
                        "$\\alpha$ and $I$ to true values of BA parameters.")
    capt <- paste(capt.short, "GLMs ere fitted with the Gaussian distribution and",
                  "inverse link function. Coefficients are interpretable as",
                  "additive effects on the inverse of the mean error.")
    print(xtable(glm.tbl, label="tab:glm", caption=c(capt, capt.short)),
          include.rownames=FALSE, sanitize.text.function=identity,
          sanitize.colnames.function=identity)
@

\add{The GLM analysis indicated that the error in estimates of $\alpha$
decreased with larger true values of $\alpha$
...
only weakly identifiable between the values of 0 and 0.5 (\cref{fig:abcptm2}).
...
<<marginals, include=FALSE>>=
    q <- 0.42
    middle <- d[alpha_error > quantile(alpha_error, q) & 
                alpha_error < quantile(alpha_error, 1-q) & 
                I_error > quantile(I_error, q) & 
                I_error < quantile(I_error, 1-q),]
    middle.plot <- middle[1,sprintf("%.1f_%.0f_%.0f_%.0f_%.0f", true_alpha, true_I, true_m, true_N, replicate)]
@
...
%\Sexpr{replace_number(nrow(middle))} with errors in both $\alpha$ and $I$
%between the \Sexpr{round(q*100)}nd and \Sexpr{round((1-q)*100)}th percentiles.
The parameters for this simulation were
    $\alpha = \Sexpr{middle[1,true_alpha]}$,
    $I = \Sexpr{middle[1,true_I]}$,
    $m = \Sexpr{middle[1,true_m]}$, and
    $N = \Sexpr{middle[1,true_N]}$.
...
\begin{figure}[ht]
    \centering
    \includegraphics[width=\textwidth]{{\Sexpr{file.path("abc-posterior", middle.plot)}}.pdf}
    \caption[
...
\begin{figure}[ht]
    \centering
    %\includegraphics[width=\textwidth]{{\Sexpr{file.path("abc-posterior-2d", middle.plot)}}.pdf}
    \caption[
...
The approximate one-dimensional marginal posterior distributions for this
...
The approximate one-dimensional marginal posterior distributions are shown in
\cref{fig:peerdriven}. Both of these results were in line with estimates
obtained on other simulated datasets (\cref{tab:abchpd}), although the estimate
of peer-driven sampling for $\alpha$ was somewhat lower than typical.

<<marginal, include=FALSE>>=
    d <- setDT(collect.data("../../simulations/abc-marginal/point-estimate/*", sep="\t"))

    joint.alpha <- d[is.na(fixed), abs(alpha-true_alpha)]
    marginal.alpha <- d[fixed == "m+N+I", abs(alpha-true_alpha)]

    joint.N <- d[is.na(fixed), abs(N-true_N)]
    marginal.N <- d[fixed == "I+alpha+m", abs(N-true_N)]

    joint.I <- d[is.na(fixed), abs(I-true_I)]
    marginal.I <- d[fixed == "N+alpha+m", abs(I-true_I)]
@
...
estimates of $m$ are apparently unaffected by marginalizing out the other
...
parameters to vary, estimates of $\alpha$, $I$, and $N$ were improved by
    \Sexpr{ round((joint.alpha - marginal.alpha) * 100 / joint.alpha) }\%,
    \Sexpr{ round((joint.I - marginal.I) * 100 / joint.I) }\%, and
    \Sexpr{ round((joint.N - marginal.N) * 100 / joint.N) }\%
when all other parameters were fixed. Neither the accuracy nor the precision of
any parameter estimates appeared to improve when more computing power was used
for SMC (\cref{fig:moresmc}).}
...
\del{We applied ABC to five published HIV datasets (\cref{tab:data}),
...
for MSM. These values were obtained with \textit{gag} for the datasets where
both \textit{gag} and \textit{env} were sequenced, but the estimates for
$\alpha$ did not change appreciably between the two genes (\cref{fig:genes}).
There was a large amount of uncertainty associated with these estimates. 95\%
HPD intervals were very wide for most datasets, often encompassing almost the
entire range from 0 to 1 (\cref{fig:abchpd}).}
...
(IDU/Canada). The HET/Botswana data had a much higher estimated $I$ value
...
estimates from the \textit{env} data were substantially higher
    (average \Sexpr{pmean1[datasets, round(mean(I))]} for \textit{gag} vs.
     \Sexpr{pmean1.env[datasets, round(mean(I))]} for \textit{env},
\cref{fig:genes}). The ratios of internal to terminal branch lengths were
lower for the \textit{env} trees than the \textit{gag} trees (average 
...
for $m \geq 2$). When $m = 1$ was disallowed, the posterior means of $m$ for
all but the HET/Botswana data took on the value 3, with the HPD intervals
spanning the entire prior region. This is very similar to the results observed
for $m$ on simulated data (\cref{tab:abchpd}), and suggests that $m$ is not
identifiable from these data with this prior. On the other hand, the estimate
and 95\% HPD interval for the HET/Botswana data (4 [2-5]) was the same for both
priors. The results for $N$ did not change appreciably between the two choices
of prior.}
...
sampling, was also identifiable, albeit slightly over-estimated with ABC for
both values we considered. Sackin's index was better able to discern $I$ from
tree shape than the nLTT (\cref{fig:rsquared,fig:Icrossv}), suggesting that
this parameter impacts the distribution of branching times in the tree more
than the topology. In a homogeneously-mixed population, branching times can be
modelled by the coalescent process~\autocite{kingman1982coalescent}, in our
case under the SI model~\autocite{volz2009phylodynamics}. Although networks are
not homogeneously mixed, the forces which affect the distributions of branching
times still apply. In our simulations, all discordant edges shared the same
...
N$), there is more prior mass on high $N$ values. 
...
transmission tree as before. We note also that \del{our accurate estimates of
$I$ may have been influenced by} this prior\del{, which} places more mass on
low $I$ values. However, the estimate of $I$ was very high for the HET/Botswana
data, suggesting that a strong enough signal in the data can
overcome the prior. \add{Furthermore, when $I$ was estimated marginally with
fixed $N$, the accuracy of the estimate improved even though there was no
...
nodes $I$ was small (\cref{fig:Ncrossv}).} $N$ was always significantly
over-estimated using ABC (\cref{fig:abcpt2}).

In retrospect, it is unreasonable to expect good estimation of $N$, because
adding additional nodes does not change the edge density or overall shape of a
BA network. This can be illustrated by imagining that we add a small number of
nodes to a network after the epidemic simulation has already been completed. It
is possible that none of these new nodes attains a connection to any infected
node. Thus, running the simulation again on the
new, larger network could produce the exact same transmission tree as before.
...
identifiable with ABC. However, all the analyses presented here did not take
the absolute size of the transmission trees into account, as the branch lengths
were rescaled by their mean. Because higher $m$ values imply higher edge
density, an epidemic should spread more quickly for higher $m$ than lower $m$
with the same per-edge transmission probability. Hence, considering the
absolute height of the trees may improve our method's ability to reconstruct
$m$. It was also pointed out to us by an anonymous reviewer that for a fixed
$I$, an infected node may only end up transmitting along a fraction of its
outgoing edges, which could mask the presence of the extra edges associated
with higher $m$.}
...
of this potential misspecification. However, we note that for \del{two}\add{the
majority} of the datasets, the estimated values of $\alpha$ did not change much
between priors, and the estimates of $I$ were robust to the choice of prior for
all datasets studied (compare \cref{fig:abchpd,fig:abchpdm2}). More
...
datasets analysed fell into three categories (\cref{fig:abchpd}). First, we
...
Our estimates of $\alpha$ for the IDU/Romania was above this range under both
priors, as were the estimates for the MSM/Beijing data and the BC data when $m
= 1$ was disallowed by the prior. The dataset investigated by
...
known phylogenetic or geographic clusters of HIV-infected persons. It is
therefore unsurprising that we detected stronger preferential attachment
dynamics in some cases. For instance, random sampling is much less likely to
discover the high-degree nodes characterizing the tail of the degree
distribution, simply because those individuals are rare in the general
population. In addition, it is plausible that HIV-positive individuals are more
likely to be highly connected in their sexual networks, as the odds of
acquiring HIV increase with the number of unprotected sexual contacts.}
...
\add{For both choices of prior on $m$, the datasets derived from IDU
populations had a higher estimated preferential attachment power than the other
datasets (\cref{fig:abchpd,fig:abchpdm2}). This finding is in line with
...
poorest identifiability (\cref{fig:abcptm2,fig:abcptm3,fig:abcptm4}). This was
reflected in the high level of uncertainty in the estimates, with most 95\%
HPD intervals covering the majority of the range [0, 1]. The value $\alpha =
0.5$ was contained in the 95\% HPD interval for every dataset; consequently, it
is not possible to say with high confidence that any of the $\alpha$ values are
different from each other. In synthetic data, the confidence intervals around
$\alpha$ narrowed when other parameters were marginalized out
(\cref{fig:marginal}). Thus, it is possible that estimates of $\alpha$ could be
made more precise by specifying either exact values or informative priors on
the other BA parameters when these are known.}

\add{I is underestimated in some cases and overestimated in others. Some of the
datasets have most of the posterior mass on m = 1. Estimates of N are
meaningless.}
...
\begin{figure}
    ...
    \caption[
        ...
        model parameter estimated marginally with ABC.
    ]{
        Posterior means (points), 50\% HPD intervals (notches), and 95\% HPD
        intervals (lines) for BA model parameter estimated marginally with ABC.
        $x$-axis labels indicate parameters which were fixed to their true
...
\begin{figure}
    ...
    \caption[
        Posterior means and 50\%/95\% HPD intervals for parameters of the
        BA network model, fitted to three HIV datasets where both \textit{gag}
        and \textit{env} genes were sequenced.
    ]{
        Posterior means (points), 50\% HPD intervals (notches), and 95\%
        HPD intervals (lines) for parameters of the BA network model, fitted to
        three HIV datasets where both \textit{gag} and \textit{env} genes were
        sequenced.
    }
    \label{fig:genes}
\end{figure}
