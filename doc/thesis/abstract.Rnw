...
attachment power \add{were all sub-linear} and ranged from \del{0.06 to 1.05}
\add{ 
    \Sexpr{d[parameter == "alpha", round(min(mean), 2)]}
to
    \Sexpr{d[parameter == "alpha", round(max(mean), 2)]}.
Point estimates of the strength of preferential attachment were higher in
injection drug user populations, potentially indicating that high-degree
``hub'' nodes may play a role in epidemics among this risk group.} Our results
underscore the importance of considering contact structures when
