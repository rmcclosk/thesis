...
\begin{figure}[H]
  ...
  \caption{
      Posterior distribution of BA model parameters for BC data. Vertical lines
      indicate maximum \textit{a posteriori} estimates, and shaded areas are
      95\% highest posterior density intervals. $x$-axis indicates regions of
...
<<abc_posterior, echo=FALSE, results="asis">>=
    ...
    for (alpha in c(0, 0.5, 1, 1.5)) {
        ...
        cat(sprintf("Approximate marginal posterior distributions of BA model parameters obtained by applying \textit{netabc} to a simulated transmission tree with BA parameter values $alpha$ = %.1f, $I$ = %d, $m$ = %d, and $N$ = %d. Vertical dashed lines indicate true values. Shaded areas are 95\\%% highest posterior density intervals. $x$-axes indicate regions of nonzero prior density.\n", alpha, I, m, N))
        cat("}\n")
