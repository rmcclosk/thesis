...
(\cref{fig:gridptI}). \add{We also note the resemblance of the contour surface
of \cref{fig:derivs} to the two-dimensional marginal posterior distributions on
\gls{I} and \gls{N} obtained with simulated data
(\cref{fig:abclow2,fig:abchigh2}). However, while fixing \gls{N} and any other
parameter resulted in much more accurate estimates of \gls{I}, fixing \gls{N}
alone did not (\cref{fig:marginal}). Therefore, there may be more complex
parameter dependence in the \gls{BA} model which we have not considered here.}
...
datasets analysed fell into two categories (\cref{fig:abchpd}). First, we
...
\gls{alpha} for these data was 
    X%\S%expr{d[m_min == 1 & dataset == "novitsky2014" & parameter == "alpha", round(mean, 2)]}
        or 
    X%\S%expr{d[m_min == 2 & dataset == "novitsky2014" & parameter == "alpha", round(mean, 2)]},
depending on the prior on \gls{m}; \textcite{de2007preferential} estimated
...
marginalized out (\cref{fig:maginal}). Thus, it is possible that estimates of
...
\cref{tab:litgamma} shows some estimated \gls{gamma} values reported in the
network literature.  The values mainly fall between 2 and 4, although
substantial variation within this range exists for networks with apparently
similar demographics. Some of these differences are likely due to the differing
time intervals considered in each study -- some investigate the distribution of
lifetime partners, while others consider only recent partnerships, such as in
the past year. TODO
...
\begin{table}[ht]
    ...
    \caption[
        ...
        these values can be assumed to reflect primarily heterosexual dynamics.
...
and/or disclosure of status. Several of the studies whose data we analysed
...
the \gls{HIV} prevalence in the analysed study populations, and relate this to
...
prior on \gls{m} (\cref{fig:abchpd,fig:abchpdm2}), we discuss here only the
results obtained with the prior $\gls{m} \sim \DiscreteUniform(1, 5)$.}

\add{The prevalence estimates for two \gls{IDU} epidemics in Eastern Europe,
IDU/Estonia and IDU/Romania, were similar 
    (X and X), 
although the confidence interval around the \textcite{niculescu2015recent} data
was wider. \textcite{niculescu2015recent} reported that 494 new \gls{HIV}
infections were diagnosed among the studied population (\glspl{IDU} in
Bucharest) during the study period, and this value was contained in our 95\%
\gls{HPD} 
    (X-X). 
On the other hand, \textcite{zetterberg2004two} collected samples from several
locations throughout Estonia; using their reported incidence estimates, there
were approximately 2000 new infections in the country during the study period.
This value was outside our 95\% \gls{HPD} 
    (X-X). 
...
\add{Surprisingly, the prevalence estimates for two \gls{MSM} populations major
Chinese cities (MSM/Shanghai and MSM/Beijing) were quite different
    (X and X).
...
prevalence obtained with \software{netabc}
    (X).
However, the authors did not claim to have sampled the entire sexual network.
In a nationwide survey, \textcite{wu2013hiv} estimated that there were 24,198
\gls{MSM} living in Beijing, of whom 5.7\%, or 1379, were \gls{HIV}-infected,
which is within the 95\% confidence interval of our estimate of \gls{I}
    (X-X).
...
network. In fact, our estimate of \gls{I} for these data
    (X)
was closer to the number of \gls{MSM} enrolled in the study, 301. The study
focused exclusively on newly diagnosed individuals; perhaps chronically
infected \gls{MSM} were no longer part of the same network. On the other hand,
our estimated prevalence for the MSM/USA data
    (X)
...
the majority of \gls{HIV} infected persons. Our estimate was slightly (TODO:
lower/higher)
    (X),
but the value 1434 was contained ith the 95\% \gls{HPD} interval
    (X-X).
...
lowest estimated \gls{I} value among the heterosexual datasets, and 200 was
contained within the 95\% \gls{HPD} interval
    (X-X),
the estimated prevalence was much higher than this value (X). Due to the
thorough population-level screening, it is most likely that the posterior mean
represents an overestimate, especially considering the observed upward bias of
\gls{ABC} estimates of \gls{I} observed on simulated data (\cref{fig:abcptm2}).
However, it is also possible that the true sexual network extended outside the
study's geographic area to include a larger number of infected individuals.
The authors of the HET/Botswana data~\autocite{novitsky2013phylogenetic, 
...
was much higher 
    (X), 
with a 95\% \gls{HPD} interval spanning nearly the entire prior region. Since
the total population of the study area was approximately 15,000, this value of
\gls{I} seems unlikely to be accurate. However, the authors noted that the town
where the study took place was located proximally to the country's capital
city, and suggested that frequent travel between the two locations may have
facilitated linking of their sexual networks. Thus, the high estimate of
\gls{I} we obtained for these data may include a larger network component based
in the capital city.}
...
would form a single, highly-connected contact network of the type generated by
the \gls{BA} model.}
