...
poorest identifiability (\cref{fig:gridest,fig:abcpt}). This was reflected in
...
overestimate, especially considering the upward bias of \gls{ABC} estimates of
\gls{I} observed on simulated data (\cref{fig:abcpt2}). It is also possible
that the true sexual network extended outside the study's geographic area to
include a larger number of infected individuals. The authors of the
