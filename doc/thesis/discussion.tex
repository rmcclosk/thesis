...
sub-linear, ranging from 
    X%\S%expr{d[m_min == 1 & parameter == "alpha", round(min(mean), 2)]}
to 
    X%\S%expr{d[m_min == 1 & parameter == "alpha", round(max(mean), 2)]}.
...
{\color{blue}\uline{The true \gls{HIV} prevalence in a population can be
...
infected individuals sampled is usually low, whic may imede our ability to
estimate \gls{I}.

The estimates of the prevalence \gls{I} were largely robust to the choice of
prior on \gls{m}} (\cref{fig:abchpd,fig:abchpdm2}). 

\textcite{niculescu2015recent} and \textcite{zetterberg2004two} both reported 
on epidemics in \gls{IDU} populations in Eastern Europe (Romania and Estonia,
respectively). The point estimates of \gls{I} for these two datasets were
similar (X and X), although the confidence interval around the
\textcite{niculescu2015recent} data was wider. \textcite{niculescu2015recent}
\uline{reported that 494 new \gls{HIV} infections were diagnosed among the
studied population (\glspl{IDU} in Bucharest) during the study period, and this
value was contained in our 95\% \gls{HPD} (X-X). On the other hand,
\textcite{zetterberg2004two} collected samples from several locations
throughout Estonia; using their reported incidence estimates, there were
approximately 2000 new infections in the country during the study period. This
value was outside our 95\% \gls{HPD} (X-X). The authors indicated that their
sample was ``convenience based''. It is possible that other, harder-to-reach
individuals had no contacts with those sampled, which would reduce the apparent
network size. Alternatively, the low estimated prevalence may have been due to
the low sampling fraction.}

\uline{The cluster we analysed within the BC data consists primarily of
\gls{IDU} in Vancouver's downtown east side. This population has been
intensively targeted by public health interventions. Combined with the routine 
clinical genotyping performed in BC for all new \gls{HIV} infections, it is not
unreasonable to suppose that the sampled individuals (399) comprise the
majority of \gls{HIV}-positive network members.

\textcite{wang2015targeting} and \textcite{li2015hiv} studied \gls{MSM}
