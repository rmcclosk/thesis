...
$\I$ is the indicator function~\autocite{marin2012approximate}.
{\color{blue}\uline{The term $\Pr(z \mid \theta)$ appears to be the bothersome
likelihood again, but this will turn out not to be a problem because we are
simulating $z$ ourselves. In fact, it is possible to sample from this
distribution exactly (see next section).}}
...
be sampled from exactly. The word ``approximate'' derives from the assumption
that,}}
{\color{blue}\uline{It is not yet obvious that the \gls{ABC} target
distribution just presented can in fact be used to approximate the true
posterior. TODO }}
...
approximation to the posterior. {\color{blue}\uline{The initial importance
distribution, denoted as $\eta_0$ in} \cref{alg:smcsamp}, \uline{is taken to be}
$\Pr(\theta) \Pr(z \mid \theta)$. TODO}
