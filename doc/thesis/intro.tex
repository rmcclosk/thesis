...
correspond to their common ancestors, \add{usually extinct (although
occasionally the internal nodes may be observed as well, eg.
...
lengths connecting node $n$ to its $\nc(n)$ children. \add{Furthermore, let
$\nl(n)$ be the number of children of $n$ which are leaves (we always have
$\nl(n) \leq \nc(n)$). The \defn{production rule} of $n$ is the pair $(\nc(n),
\nl(n))$.} That is, if two nodes have the same number of children and among
these, the same number of leaves, then they have the same production rule. Let
$k_G(x, y)$ be a Gaussian radial basis function of the vectors $x$ and $y$,
\[
...
\[
  ...
  \end{cases}
\]
\add{The parameter $\lambda$ in the above expression, is called the \emph{decay
factor}~\autocite{moschitti2006making}, and takes a value between 0 and 1.
Without this parameter, terms in the sum \ref{eq:kernel} corresponding to large
subset trees with the same topology would be similarly large and tend to
dominate the kernel score. $\lambda$ penalizes $\Delta$ more strongly as the
...
trying to estimate. \add{When evaluating the tree kernel, it is helpful to
reorder the children of each internal node such that the larger of the two
subtrees is on the right-hand side. If the two subtrees have equal sizes, then
the child with the longer branch length can be put on the right-hand side. This
operation is referred to as \defn{ladderizing}. Since the ordering of children
is arbitrary in phylogenies, this operation ensures that a maximal number of
matching subset trees are counted by the tree kernel without making meaningful
changes to the trees.}

\add{The tree kernel was later shown to be highly effective in differentiating
trees simulated under a compartmental model with two risk groups of varying
contact rates~\autocite{poon2015phylodynamic}. In that paper,
...
arbitrarily low transmissibility \del{can persist} \add{have a chance, however
small, of persisting} at low levels indefinitely. This is in contrast with
...
and sexual networks. \add{\textcite{dombrowski2013topological} contend that
sexual networks are shaped more by homophily (``like attract like'') than by
preferential attachment, but find that \gls{IDU} network do demonstrate a
scale-free structure.}

In the paper describing the \gls{BA} model, \citeauthor{barabasi1999emergence}
suggest an extension where the probability of choosing a partner of degree $d$
is proportional to $d^\alpha + 1$ for some constant \gls{alpha}.
\add{When $\gls{alpha} \neq 1$, the degree distribution no longer follows
a power law~\autocite{krapivsky2000connectivity}. For $\gls{alpha} < 1$, the
distribution is a stretched exponential, meaning that the number of nodes of
degree $k$ is proportional to $\exp(-k^\beta)$ for some constant $\beta$. For
$\gls{alpha} > 1$, the distribution takes on a characteristic called
\emph{gelation}, where a one or a few high-degree hub nodes are connected to
nearly every other node in the graph. We do not believe these departures from
the power law affect the applicability of the model to real world networks. In
fact, \textcite{de2007preferential} were able to estimate the \acrlong{PA}
power from partner count data collected from the same individuals for
consecutive time intervals, and found a value less than one in all cases. It is
also worth noting that, in addition to the \gls{BA} model, other investigations
of the interaction between contact networks and transmission trees have studied
the \acrlong{ER} and \acrlong{WS} models~\autocite{leventhal2012inferring},
whose degree distributions do not generally follow a power law under any
parameter settings.}

When $m = 1$, the network takes on the distinctive shape of a tree, that is, it
does not contain any cycles. Cycles are present in the network for all other
$m$ values. Examples of \gls{BA} networks with three different values of the
...
\add{Recall that the primary objective of our work is to develop a statistical
inference method for estimating contact network parameters from transmission
trees. For a network model with parameters $\theta$ and an input transmission
tree $T$, our goal is to estimate statistics, such as means and credible
...
\begin{align}
  ...
  \label{eq:post}
\end{align}
\add{As alluded to in \cref{sec:obj}, both the likelihood $\lik(T \mid \theta)$
and the normalizing constant are likely computationally intractable
(this will be discussed further in \cref{subsec:just}). Hence, rather than
computing the posterior distribution analytically, we will approximate it using
a \defn{Monte Carlo} approach. The fundamental idea behind Monte Carlo methods
is succinctly expressed by \textcite{liu2001theoretical}:}
\begin{quote}
    \add{Monte Carlo's view of the world is that any probability distribution
    $\pi$, regardless of its complexity, can always be \emph{represented} by a
    discrete sample from it. By ``represented'', we mean that any
    computation of expectations using $\pi$ can be replaced to an acceptable
    degree of accuracy by using the empirical distribution resulting from the
    discrete sample.}
\end{quote}
\add{In other words, if we are able to sample enough points from a distribution
of interest, we will be able to make reasonably accurate statements about the
distribution itself. For example, the expected value of the distribution can be
estimated by the sample's population mean. The reason Monte Carlo methods will
be useful in this work is that algorithms exist for obtaining samples from
distributions that are analytically intractable and from which direct sampling
is not possible (for a review see \autocite{robert2004monte}).
\Gls{SMC}~\autocite{doucet2000sequential, doucet2001introduction, liu2008monte}
is one such algorithm.}
  
\add{\Gls{SMC} considers a population of points or ``particles'', here denoted
$\set{x^{(k)}}$ and indexed by an integer $k$. The particles are associated
with weights, $\set{w(x^{(k)})}$. After the algorithm has been run, the
weighted particles are a Monte Carlo representation for the target
distribution. For example, the expected value is approximated by}
\[
  \madd{\frac{1}{n} \sum_{k=1}^n x^{(k)} w(x^{(k)}),}
\]
\add{where $n$ is the number of particles. Initially, the particles do not
...
representation of the target. \Gls{SMC} is also known as the \defn{particle
filter}.}

\add{In this work, the distribution of interest is the posterior distribution
...
weighted population was a reasonable representation of $\post(\theta \mid T)$.
In our case, due to the intractable likelihood, we will need to consider an
approximation to the posterior (see \cref{sec:abc}). However, for now, nothing
is lost by assuming that our target distribution is the posterior itself.}

\add{In this section, we review an algorithm called the \gls{SMC} sampler,
developed by \textcite{del2006sequential}, which forms the basis of the
adaptive \gls{ABC}-\gls{SMC} algorithm we apply toward the main objective of
this work. We begin by describing \gls{SIS}, which is a precursor to \gls{SMC}
that samples from a sequence of distributions defined on spaces of increasing
dimension. We then describe \gls{SMC} itself, which extends \gls{SIS} with a
resampling step to fight particle degeneracy. Finally, we outline the \gls{SMC}
sampler, which allows \gls{SMC} to be applied to sequences of distributions all
defined on the same space. This terminology will become clear as the methods
are described.}

\del{\Gls{SMC} is the name for a family of statistical inference methods that
rely on approximating probability distributions of interest with large
collections of \defn{particles}, here denoted
...
\defn{Markov kernel} as the continuous analogue of the transition matrix in a
...
sharing a name (the word ``kernel'' is ubiquitous in mathematics). \add{Also, 
the variable $\pi$ is used here for a generic target distribution to describe
the algorithm, and does not refer to the prior or posterior distributions we
will eventually want to work with.}

\subsection{Sequential importance sampling}
\label{subsec:sis}
\glsreset{SIS}

\Gls{SIS}~\autocite{gordon1993novel} \add{is a particle-based method} whose aim
...
\begin{align}
  ...
  \label{eq:is}
\end{align}
\add{Since $\eta$ can be sampled from exactly, and $\gamma$ and $f$ can both be
evaluated pointwise, the integral $\int w(x) \eta(x) f(x) \d x$ can be
approximated by a Monte Carlo estimate. We simply take a sample from $\eta$,
multiply each point in the sample by $f$ and $\gamma$ evaluated at that point,
and sum the results. Moreover, the normalizing constant $Z$ can be expressed in
terms of the importance weight and distribution, $Z = \int w(x) \eta(x) \d x$.
Therefore, we have all the ingredients we need to obtain an estimate of
...
sample from \add{$\eta$ and using it to obtain an approximate sample from $\pi$
by \gls{IS}} (\cref{alg:sis}). We begin with $n$ particles, which have been
sampled from the importance distribution $\eta(x_1)$ for $\pi(x_1)$. \del{The
...
\add{Each particle is extended by drawing $x_2$ from $\eta(x_2 \mid x_1)$, and
the importance weights are updated by \cref{eq:sisw}. This procedure is
repeated $d$ times until the particles have dimension equal to that of $\pi$.}
...
\begin{algorithm}
  ...
  \begin{algorithmic}
    \For {$k = 1$ to $n$}
      ...
      \Comment Initialize importance weight
    \EndFor
    \For {$i = 2$ to $d$}
      \For {$k = 1$ to $n$}
...
weights after all $d$ sequential steps, \add{a problem known as \defn{particle
degeneracy}. To mitigate this problem, \textcite{gordon1993novel} introduced
technique they called the \defn{bootstrap filter}, which involves a resampling
of the population of particles after each sequential step in accordance with
their importance weights. A similar idea, termed \defn{particle rejuvenation},
was proposed by \textcite{liu1995blind}. These approaches cause particles with
high importance weights to be replicated in the population, while particles
with low weights may be removed. After each resampling step, the importance
weights for all particles are set equal.}

\add{The resampling step was formally integrated with \gls{SIS} by
\textcite{doucet2000sequential} to form the first \gls{SMC} algorithm
(\cref{alg:smc}). Rather than resample at every step as the bootstrap filter
proposed, the authors use a criterion based on the \gls{ESS} the particle
population to determine when resampling is necessary.} The \gls{ESS} of the
...
triggered when the \gls{ESS} drops below the threshold \add{(conventionally
$n/2$~\autocite{liu2008monte})}. \del{This results in the removal of low-weight
particles from the population, and also equalizes all the weights.} Various
...
developed the \defn{\gls{SMC} sampler} with an alternative objective: to sample
...
\]
of increasing dimension. \add{That is, $\tilde{\pi}_i$ has dimension equal to
the original dimension of $\pi_i$ raised to the power of $i$.} This brings us
...
distributions. \add{The dimension of the importance distributions $\eta$ is
similarly augmented with the forward kernels,}
\begin{align}
  \eta_i(\vec{x_i}) = \eta_1(x_1) \prod_{k=1}^{i-1} K_k(x_k, x_{k+1})
  \label{eq:smcimp}
\end{align}
\add{Thanks to the backwards kernels, we can write $\tilde{\pi}_i$ in terms of
$\tilde{\pi}_{i-1}$ as follows.}
\begin{align*}
...
\end{align*}
\add{The importance distributions can also be expressed recursively using the
forward kernels, which follows directly from \cref{eq:smcimp},}
\[
...
\add{In fact, since the weight update \cref{eq:smcwt} depends only on the $i$th
and $i-1$st components of each particle, we do not even need to keep track of
the complete particles if we are only interested in the final distribution.}
\del{These are all the ingredients we need to apply \gls{SIS}.} The sequences
...
called the \defn{optimal} backward kernels. The full \gls{SMC} sampler
...
\begin{algorithm}
  ...
  \begin{algorithmic}
    \For {$k = 1$ to $n$}
      ...
      \Comment{Initialize the importance weights}
      \State Normalize the weights so that $\sum w = 1$
    \EndFor
    \For {$i = 2$ to $d$}
      \For {$k = 1$ to $n$}
        ...
        \Comment{Update the importance weights}
      \EndFor
      \State Normalize the weights so that $\sum w = 1$
      \If {$\ESS(w) < T$}
        \Comment{$T$ is a user-defined threshold}
...
\add{\Acrlong{SMC}, and the \gls{SMC} sampler, were developed for sampling from
distributions which can be evaluated up to a normalizing constant. We claim,
and shall argue more thoroughly below (\cref{subsec:just}) that the posterior
distribution}
\[
  \madd{\post(\theta \mid T) \propto \lik(T \mid \theta) \prior(\theta)}
\]
\add{for a contact network model with parameters $\theta$ and an input
transmission tree $T$ does not fall in this category. Therefore, \gls{SMC}, and
other Bayesian and \gls{ML} techniques for fitting mathematical models (see
\cref{chp:prelim}), cannot be directly applied to our problem. In particular,
\gls{MCMC} and the \gls{SMC} sampler are designed for distributions $\pi$ which
can be evaluated up to a normalizing constant $Z$, that is, $\pi(x) = \gamma(x)
/ Z$. Both algorithms calculate a ratio of the form $\pi(x) / \pi(x') \propto
\gamma(x) / \gamma(x')$ for a current value $x$ and proposed updated value $x'$
- for \gls{MCMC}, this is part of the \acrlong{MH} ratio, while for the
\gls{SMC} sampler, a similar ratio is required to calculate the importance
weights. In the context of Bayesian inference, this ratio contains a likelihood
ratio, which must be calculated by computing the individual likelihoods and
dividing them. If the likelihood is intractable, this is clearly not a viable
approach.}

\add{\Gls{ABC}~\autocite{rubin1984bayesianly, tavare1997inferring,
fu1997estimating} was developed to estimate posterior distributions with
intractable likelihoods, which have arisen frequently in the domain of
population genetics~\autocite{beaumont2002approximate,
beaumont2010approximate}. \Gls{ABC} navigates around the intractable likelihood
by replacing the posterior as the target of inference by an \emph{approximate}
posterior. This distribution is constructed in such a way that the ratios
required for \gls{MCMC} and the \gls{SMC} sampler can be computed, conveniently
allowing us to apply those algorithms with minimal changes. In the next
section, we shall demonstrate how this is done, but first we give the
definition of the approximate posterior.}

\del{Most mathematical models are amenable to fitting via one or both of the
...
\del{Ordinarily, Bayesian inference targets the posterior distribution
$\post(\theta \mid y)$. That is, in the Bayesian framework,}
\add{By targeting the posterior distribution, Bayesian inference makes the
assertion that} model parameters with higher posterior density are ``better''
...
generating the observed data. More formally, \add{let $y$ be the observed data
to which we are trying to fit a model with parameters $\theta$. In the case of
this work, the data is a transmission tree $T$, but we shall stick with the
generic variable $y$ for now.} Suppose we have a distance measure $\rho$
...
\textit{\gls{ABC} target distribution}. \add{The term $\lik(z \mid \theta)$
appears to be the bothersome likelihood again, but this will turn out not to
be a problem because we are simulating $z$ ourselves.}

\add{To return to the context of this thesis, the observed data $y$ is an
estimated transmission tree for a viral epidemic under investigation. The model
in question is a contact network model with parameters $\theta$. The simulated
dataset $z$ is a transmission tree, obtained by first generating a contact
network under the model, and then simulating the spread of an epidemic over
that network. A transmission tree can be constructed by keeping track of who
infected whom during the simulated epidemic (further details will be given in
\cref{subsec:nettree}). The distance function $\rho$ must compare two
transmission trees - the observed tree, which takes the place of $y$, and the
simulated tree $z$. We will define this distance function using the tree kernel
discussed in \cref{subsec:treeshape}. In words, the approximate posterior we
consider here is a distribution which assigns a joint probability density to
model parameters and simulated transmission trees under those parameters. The
probability density is proportional to the product of the prior on the
parameters, and the likelihood of the simulated transmission tree under those
parameters, but only if the simulated transmission tree is sufficiently close
to the true tree.  Otherwise, the probability density is zero.}

\del{As we shall see in the next section, this distribution can be sampled from
exactly. The word ``approximate'' derives from assumption that, for a suitably
chosen distance $\rho$ and a small enough $\varepsilon$, the marginal in $z$ of
this distribution approximates the posterior of
interest~\autocite{marin2012approximate}. That is,}
\add{In fact, it is not the \gls{ABC} target distribution
itself, but rather its marginal in $z$, which approximates the posterior
distribution. In other words, we claim that}
\[
  \int \pi_\varepsilon(\theta, z \mid y) \d z \approx \post(\theta \mid y).
\]
The intuition for why this approximation might be reasonable comes from
\add{considering the case when $\varepsilon$ = 0 and $\rho$ has the property
that $\rho(z, y)$ if and only if $x = y$. In that case,} \del{the fact that,
when $\varepsilon = 0$,} the $\varepsilon$-ball around $y$ should contain only
$y$ itself, hence the integral on the left is exactly equal to the posterior.
Thus, by taking $\varepsilon$ small, we should attain something close to the
posterior \add{if $\rho$ captures the similarity between datasets reasonably
well. However, the accuracy of the \gls{ABC} approximation depends heavily on
the choice of distance function
\autocite{aeschbacher2012novel,blum2013comparative}.}

\subsubsection{Distance functions and summary statistics in ABC}

In many applications \add{(eg. \autocite{fu1997estimating,tanaka2006using})},
$\rho$ is defined as $\rho(S(\cdot), S(\cdot))$ where $S$ is a function which
maps data points into a vector of summary statistics.
\add{In the context of \gls{ABC}, a summary statistic $S$ is called
\defn{sufficient} if}
\[
  \post(\theta \mid y) = \post(\theta \mid S(y)).
\]
\add{That is, sufficiency implies that the data can be replaced with the
...
\autocite[][and~references~therein]{aeschbacher2012novel,blum2013comparative}.
We do not apply these methods in this work, instead focusing on a distance
function which is not based on summary statistics.}

\del{Summary statistics can be useful if the data are high-dimensional or of a
complex type, but}\add{Although summary statistics are often presented as a
fundamental part of \gls{ABC},} they are not strictly necessary. For instance,
if the data are numeric and of low dimension, the distance function may simply
be the Euclidean distance~\autocite{sisson2007sequential}.
\add{\textcite{park2015k2} proposed the use of a kernel function
(as defined in \cref{subsec:treeshape}) in place of a distance function.  The
authors referred to their approach as ``double-kernel \gls{ABC}'' due to the
use of a second (unrelated) kernel function to compute the weights of the
particles. The work by \textcite{poon2015phylodynamic}, upon which ours is
based, employed a similar approach, replacing the likelihood ratio in Bayesian
\gls{MCMC} with a ratio of kernel scores.}

\subsection{Algorithms for ABC}
\label{subsec:abcalg}

Algorithms for performing \gls{ABC} can be grouped into three categories:
...
target distribution can be written $\pi_\varepsilon(x \mid y)$. \add{This makes
our notation consistent with \cref{sec:smc}.}
...
approximation to the posterior. \add{The initial value $\varepsilon_1$ is set
to $\infty$, which makes the first distribution in the sequence}
\[
  \madd{\pi_1(\theta, z) = \frac{\prior(\theta) \lik(z \mid \theta)}
  {\int_{\R \times \Theta} \prior(\theta) \lik(z \mid \theta) \d\theta \d z}}.
\]
\add{This initial distribution does not depend on the observed data $y$.  In
the terminology of the \gls{SMC} sampler (\cref{alg:smc}), the numerator is the
first of the $\gamma$'s, that is, $\gamma_1 = \prior(\theta) \lik(z \mid
\theta)$. Sampling in proportion to $\gamma_1$ is straightforward and was
already demonstrated for rejection \gls{ABC} above. Because the sampling is
exact, the initial importance weights are all set equal to 1 and normalized to
$1/\gls{n}$ where \gls{n} is the number of particles.}

As discussed in \cref{subsec:smcsamp}, the choices of the kernels $K$ and $L$
...
sisson2007sequential, del2012adaptive}. \add{With an appropriately chosen
kernel, the weight update (\cref{eq:smcwt}) will simplify into a computable
expression. \textcite{sisson2007sequential} and \textcite{beaumont2009adaptive}
both suggest random walk kernels for $K$, where each particle is perturbed
according to a Gaussian distribution. The backwards kernels $L$ are chosen to
approximately minimize the variance in importance weights. In this thesis, we
use an \gls{MCMC} kernel, as proposed by \textcite{del2012adaptive}.  The
associated backwards kernels and weight updates are discussed in the next
chapter (\cref{subsec:adaptsmc}). With generic kernels $K$ and $L$, the
\gls{ABC}-\gls{SMC} algorithm is almost identical to the \gls{SMC} sampler
(\cref{alg:smcsamp}), with $\gamma_i$ replaced with $\pi_i$.  Therefore we will
not repeat the full algorithm here.}
...
\add{In \cref{sec:obj}, we outlined three research objectives that will be
addressed in this thesis. First, we aim to develop a method for fitting contact
network models to estimated transmission trees. Although transmission trees
can be estimated for any epidemic by thorough contact tracing, viral diseases
are the most useful context for this method due to the possibility of inferring
the trees from sequence data. Transmission and sequence evolution occur on
similar time scales for RNA viruses, resulting in viral phylogenies whose
shapes are heavily constrained by the transmission process. The study of this
interaction between evolution and epidemiology is called \emph{phylodynamics};
phylodynamic methods make it possible to estimate transmission trees from viral
sequence data. These estimated trees form the input data for our method.}

\add{The desired output of our method is a posterior distribution of the
parameters of a contact network model. Rather than assuming a homogeneously
mixed population, as most epidemiological models do, network models take the
more realistic view that human populations are structured. For example,
contacts may tend to occur more often between ``like'' individuals (with
respect to geometry, socioeconomic status, or other factors) than at random. A
network model provides a formal representation of this structure. In
particular, our second research objective is to characterize our ability to fit
the \gls{BA} model, which incorporates preferential attachment to generate
networks with realistic degree distributions.}

\add{To fit these models, our method will apply \gls{ABC}, a simulation-based
approach. Simulating a transmission tree according to a network model is
straightforward: a network can be generated according to the model, and the
spread of an epidemic can be simulated over the network and recorded in a
transmission tree. \Gls{ABC} uses the concordance between these simulated
...
\add{\Gls{ABC} can be implemented by at least three classes of algorithm, but
the one we choose to apply in this work is \gls{SMC}. \Gls{SMC} uses a
population of parameter ``particles'' to approximate a distribution of
interest, in this case the approximate posterior distribution targeted by
\gls{ABC}. After running the \gls{ABC}-\gls{SMC} algorithm, statistics on the
model parameters can be approximated by the weighted population of particles.
For example, a weighted average would give an approximate expected value for
each parameter.}

\add{Thus, our method integrates four research topics: phylogenetics, contact
networks, \acrlong{SMC}, and \acrlong{ABC}. The first two topics together form
the problem domain. Phylogenetic data are the input to our method, while
