...
\begin{quote}
    Monte Carlo's view of the world is that any probability distribution $\pi$, 
    ...
    sample.
\end{quote}
In other words, if we are able to sample enough points from a distribution of
interest, we will be able to make reasonably accurate statements about the
distribution itself. For example, the expected value of the distribution can be
estimated by the sample's population mean. Several algorithms exist for
obtaining such a sample, some of which will be discussed further in
\cref{subsec:mfit}. The particular method we apply in this work is \gls{SMC},
which is also known as the \emph{particle filter}. The reason for this name is
that the desired number of samples from the target distribution is decided at
the outset, and that number of ``particles'' \ldots TODO

This is in contrast 
}

\uline{In the
context of this work, the distribution of interest is the posterior
distribution on the parameters of a contact network model, given observed
phylogenetic data in the form of a transmission tree.}

\uline{A typical Bayesian Monte Carlo approach to this problem would be to
calculate the product of the likelihood and the prior for each of the parameter
particles. 
...
\gls{SIS} by \citeauthor{doucet2000sequential} to form the first \gls{SMC}
