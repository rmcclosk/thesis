...
\gls{SIS} applies \gls{IS} to the problem of sampling from a distribution $\pi$ 
...
distribution is known as the \defn{optimal
kernel}~\autocite{cappe2007overview}. Choosing each element of the
...
evidence. As with \gls{SIS}, we assume that $\pi_i = \gamma_i / Z_i$, where
$\gamma_i$ is known pointwise and the normalizing constant $Z_i$ is unknown.

Both algorithms follow the same general pattern: the target distribution is
attained by progressing through a sequence of intermediate distributions. For
\gls{SIS}, the intermediate distributions are lower-dimensional marginals of
the target, while for the \gls{SMC} sampler, they are of the same dimension and
constitute a smooth progression from an initial distribution to the target. In
both cases, the neighbouring distributions in the sequence are related to each
other in some way, and we can take advantage of that relationship to create a
sequence of importance distributions alongside the sequence of targets. In
\gls{SIS}, the neighbouring marginals $\pi(\vec{x_i})$ and
$\pi(\vec{x_{i+1}})$ were related by the conditional density $\pi(x_i \mid
\vec{x_{i-1}})$, which we used to inform the importance distribution. 

The sequence of targets considered by the \gls{SMC} sampler are assumed to be
closely enough related, that we can shift an importance distribution for
$\pi_i$ into one for $\pi_{i+1}$ by perturbing the particles in a systematic
way.
...
Ordinarily, Bayesian inference targets the posterior distribution $\Pr(\theta
