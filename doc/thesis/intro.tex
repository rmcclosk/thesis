...
\begin{align}
  \madd{\post(\theta \mid T) = \frac{\lik(T \mid \theta) \prior(\theta)}
    {\int_\Theta\lik(T \mid \theta) \prior(\theta)}.}
  \label{eq:post}
\end{align}
...
\add{Here we describe an algorithm called the \gls{SMC} sampler, developed by
\textcite{del2006sequential}, which forms the basis of the adaptive
\gls{ABC}-\gls{SMC} algorithm we apply toward the main objective of this work.
We begin by describing \gls{SIS}, which is a precursor to \gls{SMC} that
samples from a sequence of distributions defined on spaces of increasing
...
more realistic view that human populations are structured. That is, contacts
which allow for transmission may occur in a nonrandom way, rather than between
every pair of individuals in the population. A network model parameterizes this
structure. In particular, our second research objective is to characterize our
ability to fit the \gls{BA} model, which incorporates preferential attachment
to generate networks with realistic degree distributions.}
