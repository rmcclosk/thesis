...
I wrote a computer program called \software{netabc} to perform statistical
...
The algorithm may be stopped when the tolerance reaches a user-defined final
...
using the \software{ape} package. Our \software{treekernel} binary was used to
...
\software{treestat} binary, and stored them in a second $300 \times 300$
matrix.
...
\software{kernlab} package~\autocite{karatzoglou2004kernlab} to create a
\gls{kSVM} classifier using the computed kernel matrix. Second, we used the
\software{e1071} package to create an ordinary \gls{SVM} classifier using the
...
\software{HPDinterval} function in the \software{coda} package.
...
using the \software{HPDinterval} function in the \software{coda} package.
