...
node, also known as its \defn{production rule}, is recorded. The nodes of both
...
\[
  q_i(\theta, \theta') \prod_{k=1}^M \Pr(X_i^{(k)}' \mid \theta'),
\]
where $\theta$ is the vector of model parameters and $X_k$ are $M$ datasets
simulated according to $\theta$. In our implementation, $q$ is a Gaussian
proposal with variance equal to twice the empirical variance of the particles,
following~\autocite{beaumont2009adaptive, del2012adaptive}. The backwards
kernels are
\[
  L_{i-1}(z', z) = \frac{\pi_n(z)K(z, z')}{\pi_n(z')}.
\]
When substituted into \cref{eq:smcwt}, the forward kernels $K(z, z')$ and
densities $\pi_n(z')$ cancel out, and we are left with the weight update
\begin{align*}
  w(z') 
    &\propto w(z) \frac{\pi_n(z \mid y)}{\pi_{n-1}(z \mid y)} \\
    &= w(z) \frac{\pi(z) \pi_n(y \mid z)}{\pi(z) \pi_{n-1}(y \mid z)} \\
    &= w(z) \frac{\sum_{k=i}^M \I_{A_{\varepsilon_n, y}}(x_k)}
            {\sum_{k=i}^M \I_{A_{\varepsilon_{n-1}, y}}(x_k)}.
\end{align*}
...
\software{kernlab} package~\autocite{zeileis2004kernlab} to create a \gls{kSVM}
...
using \software{FastTree2} version 2.1.7 with the \gls{GTR} model. Transmission
