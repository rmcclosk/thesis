...
quantitative information about network structures from viral phylogenies.}

\add{There are three aspects to ``usefulness''. The first is model
specification and the biological or epidemiological interpretation of the
parameters. We want the model to be somewhat realistic, so that \ldots, but no
too complicated that it becomes difficult to interpret. At least some of the
parameters should be of interest from a theoretical or practical perspective.
As this is a phylodynamic method, intended to be used with viral sequence data, 
we would also like to choose parameters which may be difficult to estimate with
more standard methods; otherwise, our method provides no advantage. The
\gls{BA} model (\cref{subsec:pa}) satisfies these criteria, albeit some better
than others. The purported realism of the model stems from the fat-tailed
degree distributions it produces, which are similar to those observed in real
world sexual networks~\autocite{colgate1989risk, liljeros2001web,
schneeberger2004scale, clemenccon2015statistical, rothenberg2007large}.
Moreover, the ``rich get richer'' phenomenon, where popular individuals attract
new connections at an elevated rate, is intuitively reasonable for both
sexual~\autocite{de2007preferential} and
...
first three of these are parameters of the model itself, while \gls{I} is
...
similar metric for preventative measures. The average degree, controlled by
\gls{m}, may have some value in predicting how fast an epidemic will spread.
When there are more connections in the network, the time intervals between
transmission events decrease, as does the number of transmissions required for
the epidemic to travel from one part of the network to another. High average
degrees may be reduced by interventions causing behaviour change, such as using
condoms or caughing into one's sleeve; these types of interventions will be
less helpful when the average number of contacts per person is already low. The
preferential attachment power \gls{alpha} is perhaps the least obviously
useful, but it can offer some insight into the dynamics of the network and how
it would react to the removal of nodes. \textcite{dombrowski2013topological} 
found evidence of preferential attachment in \gls{IDU} networks, and suggested
that as a consequence of this characteristic, the removal of random nodes (such
as through a police crackdown) might inadventently make it easier for epidemics
to spread. When individuals with only one or two connections lose them, they
might tend to seek out well-known (that is, high-degree) members of the
community, thus increasing those individuals' connectivity even further.}

\add{All four \gls{BA} parameters can be estimated using more conventional
approaches, but the difficulty of doing so varies between the parameters and
based on other characteristics of the epidemic under consideration. All network
parameters can theoretically be estimated by explicitly constructing the
contact network, although this is highly resource intensive and is hampered by 
misreporting and other challenges~\autocite{eames2015six}. In any event, if we
are in a position to know the true contact network then the point of estimating
network parameters is moot. Depending on the context, \gls{N} may be estimable
with a survey, but if the population in question is ``hidden'' (such as
\gls{IDU} in regions where drug use is illegal) estimation of numbers may be
more challenging. Respondent-driven
sampling~\autocite{heckathorn1997respondent} was developed to investigate such
populations. The prevalence \gls{I} may be estimated from reports of hospital
admissions or engagement in care, but this may be very difficult for diseases
such as \gls{HCV} which may remain asymptomatic for decades. Routine
population-level testing is another possibility, but is resource intensive and
the barriers associated with hidden populations still apply. The average degree
of the network ($2\gls{m}$) can be ascertained by simply asking a sample of
people how many contacts they had, but they might be unwilling to disclose this
information or, in the case of airborne transmissions, they might not know. The
estimation of \gls{alpha} is even more complex, as this parameter is most
strongly reflected in the connectivity of very high degree nodes, who are rare
in the population. Even if full degree distributions are available, there are
models other than preferential attachment which can produce scale-free 
...
power law degree distribution. In short, each of these parameters may be
estimated without phylodynamics, but there are sufficient difficulties that we
believe an alternative method based on sequence data is warranted.} 

\add{Regardless of how useful or interesting the \gls{BA} model parameters are, 
there is another aspect to the ``usefulness'' of our method - identifiability. 
Roughly speaking, the identifiability of a parameter says how much information
about that parameter can possibly be obtained from the observed data (see
\cref{chp:prelim}). If the parameters of the \gls{BA} model do not influence
tree shape at all, then the posterior distribution will exactly resemble the
prior. In that case, no matter how accurate a representation of the posterior
we are able to produce, no useful information can be learned about the network.
Hence, before proceeding with a full validation of \software{netabc} on
simulated data, we undertook two experiments designed to assess the
identifiability of each parameter separately.}
...
corresponding parameter when we attempt to estimate it with \gls{ABC}. In this
experiment, we fix all but one of the parameters at a time, so it is possible
that identifiability will be lost when all parameters are free to vary. This
experiment also has the secondary purpose of validating our choice of the tree
kernel as a distance measure in \gls{ABC}. To tell the trees apart, we use a
classifier based on the tree kernel, but we also test two other tree shape
statistics: Sackin's index, which considers only the topology, and the
\gls{nltt}, which considers only branching times. Since the tree kernel
incorporates both of these sources of information, we expect it to outperform 
the other two statistics. Finally, the tree kernel can be ``tuned'' by
adjusting the values of the meta-parameters \gls{lambda} and \gls{sigma}. We
used this experiment to choose values for these meta-parameters which were best
...
and credible intervals of the parameter in question could be calculated. We
still held all but one parameter fixed, as the curse of dimensionality made it
infeasible to perform grid search on the full parameter space. Repeating this
...
trees were simulated under several combinations of parameter values; we tried
to recover these values by running \software{netabc} on the simulated trees.}

\add{We have argued that the \gls{BA} parameters are worth estimating, and
described three simulation-based experiments to assess our ability to do so. 
There is a final aspect of the usefulness of these estimates: the accuracy of
the \gls{ABC} approximation to the posterior.}

... There are two factors which can affect the accuracy of the \gls{ABC}
approximation: the choice of distance function, and the settings used for
\gls{SMC} such as the number of particles and \gls{ESS} decay rate
\gls{alphaess}. Fortunately, these are both easy to test. If the \gls{ABC}
estimates are wrong, we can check if the distance function is responsible by
simulating several trees under both the estimated and true values. ...
For the \gls{SMC} parameters, we can simply rerun one simulation with better
settings (more particles, more samples per particle, and higher
\gls{alphaess}), and see if the approximation improves.
...
\add{Using synthetic data,} we investigated four parameters related to the
\gls{BA} contact network model, 
denoted \gls{N}, \gls{m}, \gls{alpha}, \gls{I}
(see \cref{subsec:pa}). The first three of these are parameters of the model
itself, while \gls{I} is related to the simulation of transmission trees over
the network. However, we will refer to all four as \gls{BA} parameters. \gls{N}
denotes the total number of nodes in the network, or equivalently, susceptible
individuals in the population. When a node is added to the network, \gls{m} new
undirected edges are added incident to it, and are attached to existing nodes
of degree $d$ with probability proportional to $d^{\gls{alpha}} + 1$
(\cref{subsec:pa}). To simulate transmission trees over a \gls{BA} network, we
allowed an epidemic to spread until \gls{I} nodes were infected, and sampled a
transmission tree at that time. The \gls{alpha} parameter is unitless, while
\gls{m} has units of edges or connections per vertex, and \gls{N} and \gls{I}
both have units of nodes or individuals.
...
\begin{table}[ht]
  ...
  \caption[Variables used in grid search experiments]
  {
    ...
    \gls{ABC} was used to re-estimate posterior distributions for the
...
obtained with ABC on simulated data. The estimates shown
...
\begin{table*}[ht]
  ...
  \caption[
      ...
      Average posterior mean point estimates and 95\% highest posterior density
      (HPD) interval widths for BA model parameter estimates obtained with
      ABC. Three transmission trees were simulated under each
...
\begin{figure}[ht]
  ...
  \caption[
    ...
    with ABC for a single simulated transmission tree.
  ]{
    Marginal posterior distributions of BA model parameters estimated
...
\gls{ABC}. Therefore, our next step was to use
...
assess the robustness of \software{netabc} to the particular \gls{HIV} gene
...
We applied \software{netabc} to {\color{red}\sout{five}}
...
\Cref{fig:abchpdm2} \uline{shows point estimates and \glspl{HPD} obtained when
