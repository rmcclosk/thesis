...
\cref{tab:glmalpha,tab:glmI}. The \gls{GLM} were fitted using the inverse link
function. That is, if $p$ is the true value of the parameter and $\hat{p}$ is a
random variable representing our estimate of the parameter, the \gls{GLM}
posits a relationship of the form}
\[
...
of their mass around the mode. When the value $\gls{m} = 1$ was disallowed by
the prior, the separation between the \gls{IDU} datasets and the others became
more striking (\cref{fig:abchpdm2}). All three \gls{IDU} datasets had estimated
