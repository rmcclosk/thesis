...
{\color{blue}\uline{
This chapter is organized as follows. One section is devoted to each of our
...
of the regions of nonzero prior density. Average \gls{HPD} interval widths were
    \Sexpr{d[,round(mean(alpha.upper - alpha.lower), 2)]},
    \Sexpr{d[,round(mean(I.upper - I.lower))]},
    \Sexpr{d[,round(mean(m.upper - m.lower), 2)]}, and
    \Sexpr{d[,format(round(mean(N.upper - N.lower)), digits=5)]},
representing 
    \Sexpr{d[,round(mean(alpha.upper - alpha.lower) * 100 / 2)]}\%,
    \Sexpr{d[,round(mean(I.upper - I.lower) * 100 / 4500)]}\%,
    \Sexpr{d[,round(mean(m.upper - m.lower) * 100 / 6)]}\%, and
    \Sexpr{d[,round(mean(N.upper - N.lower) * 100 / 14500)]}\%
