...
{\color{blue}\uline{In the original paper of }\textcite{del2012adaptive}, \uline{the
parameter is called $\alpha$, but to avoid confusion with the \gls{BA}
parameter of the same name, we will refer to it here as $\alpha_{\ESS}$.}}
...
type~\autocite{murray2012mcmc}, which would imply that these models are not
...
estimating all parameters with \gls{ABC}} (\cref{subsec:abc}).
...
(see \cref{sec:treeshape}).  \uline{In contrast with Sackin's index, the
