...
sampled tips had a strong impact or the accuracy of the \gls{kSVR}
(\cref{fig:mcrossv}).}
...
misleading if the true population was only of size 100, but almost negligible
...
<<supp_labels, echo=FALSE, results="asis">>=
    ...
    worst.alpha <- d[,mean(alpha_error), by=true_alpha][,true_alpha[which.max(V1)]]
    best.alpha <- d[,mean(alpha_error), by=true_alpha][,true_alpha[which.min(V1)]]
    worst.I <- d[,mean(I_error), by=true_I][,true_I[which.max(V1)]]
    best.I <- d[,mean(I_error), by=true_I][,true_I[which.min(V1)]]
    worst.m <- d[,mean(m_error), by=true_m][,true_m[which.max(V1)]]
    best.m <- d[,mean(m_error), by=true_m][,true_m[which.min(V1)]]

    worst <- d[true_alpha == worst.alpha & true_m == worst.m & true_I == worst.I & replicate == 0,]
    best <- d[true_alpha == best.alpha & true_m == best.m & true_I == best.I & replicate == 0,]
...
marginalizing out the other parameters, corroborating the previous experiments'
...
prior to $\gls{m} \geq 2$ (compare \cref{fig:abchpd,fig:abchpdm2}), although 
