...
and at least one other variable were marginalized out. Neither the accuracy nor
the precision of any parameter estimates appeared to improve when more
computing power was used for \gls{SMC} (\cref{fig:moresmc}).}
...
<<realdata, include=FALSE>>=
  ...
  stopifnot(pmean1[,dataset[which.max(I)]] == "zetterberg2004")
  stopifnot(pmean1[,dataset[which.min(I)]] == "novitsky2014")
  stopifnot(pmean1[dataset != "novitsky2014", dataset[which.min(I)]] == "bctree")

  stopifnot(pmean1[,cor.test(ntip, I)$p.value, method="spearman"] > 0.05)
@
...
prevalence (Spearman correlation, 
    $p \Sexpr{pp(pmean1[,cor.test(ntip, I)[["p.value"]], method="spearman"], eq=TRUE)}$).
This indicates that the higher prevalence estimates were not simply due to
...
higher (\cref{fig:genes}). The ratios of terminal to internal branch lengths
were higher for the \textit{env} trees than the \textit{gag} trees (average X
for \textit{env} and X for \textit{gag}). As with \gls{alpha}, many of the
posterior means were outside the 50\% \gls{HPD} intervals, suggesting diffuse
posterior distributions. The values of \gls{I} were largely robust to
restricting the prior to $\gls{m} > 1$ (compare
\cref{fig:abchpd,fig:abchdpm2}), although TODO}
...
of the datasets analysed. The widths of the 95\% \gls{HPD} varied from 0 (all
the mass on the estimated value) to 5 (the entire prior region). When the value
$\gls{m} = 1$ was disallowed by the prior, TODO.}
...
allowed by the prior. The results did not change appreciably (TODO) when the
value $\gls{m} = 1$ was assigned zero prior probability.}
...
$\gls{m} = 1$}. Because the estimates of \gls{N} were uninformative, we
simulated another set of networks with the estimated \gls{I} value, rather than
\gls{N}, as the number of nodes. The results are summarized in
\cref{tab:gamma}. All of the estimated exponents were in the range $2 \leq
\gls{gamma} \leq 2.5$, which is on the lower end of the range $2 \leq
\gls{gamma} \leq 4$ reported in the literature.
