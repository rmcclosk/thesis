...
<<treestats, include=FALSE, eval=FALSE>>=
...
<<classifiers, include=FALSE, eval=FALSE>>=
...
<<gridsearch, include=FALSE, eval=FALSE>>=
...
<<abc_glm, include=FALSE, eval=FALSE>>=
...
<<glm_tables, echo=FALSE, warning=FALSE, error=FALSE, results="asis", eval=FALSE>>=
    source("global.R")
...
<<realdata, include=FALSE, eval=FALSE>>=
  ...
  d1 <- fread(rownames(m)[m$m_min == 1 & m$gene == "gag"])
  d2 <- fread(rownames(m)[m$m_min == 2 & m$gene == "gag"])

  d1.env <- fread(rownames(m)[m$m_min == 1 & m$gene == "env"])
  d2.env <- fread(rownames(m)[m$m_min == 2 & m$gene == "env"])
