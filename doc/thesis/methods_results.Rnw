...
$\lik(\gls{T} \mid \theta)$ is also likely to be intractible in our case.}
...
\emph{doubly-intractible} type~\autocite{murray2012mcmc}, which would imply that
these models are not amenable to neither \gls{ML} nor Bayesian inference
techniques. Although both methods are able to cope with an intractible
...
number~\autocite{britton2002bayesian}. \gls{R0} quantifies the number of
secondary infections ultimately caused by one infected individual; higher
...
population, the proportion of the population which must be vaccinated to
control an epidemic can be expressed in terms of
\gls{R0}~\autocite{anderson1982directly}. Although optimal vaccination
strategies may differ in heterogeneous contact
...
structure. Superspreaders have been hypothesized to play a greater-than-average
role in the spread of several diseases~\autocite{stadler2013uncovering,
...
crackdown) might inadventently make it easier for epidemics to spread. When
...
those individuals' connectivity even further.}

\add{All four \gls{BA} parameters can be estimated using more conventional
approaches, but this estimation may be challenging depending on characteristics
of the epidemic under consideration. In theory, any network parameter can be
estimated by explicitly constructing the contact network, although this is
highly resource intensive and is hampered by misreporting and other
challenges~\autocite{eames2015six}. All parameters become more difficult to
estimate when the infected population is ``hidden'' due to illegal or
stigmatized behaviour, as is sometimes the case with \gls{HIV} outbreaks among
\gls{IDU}, \gls{MSM}, or sex workers. Of course, phylodynamic methods are
equally ineffective when the population is completely hidden, since we must at
least be able to sample viral sequences from its members. If a population is
accessible, \gls{N} and \gls{I} may be estimable by surveying and/or testing
each member of the population for the disease. However, this will not tell us
if there are large compartments of the population that we simply have not
sampled. Our hope is that estimating \gls{N} and \gls{I} phylogenetically might
provide this additional information. The average degree of the network,
$2\gls{m}$, is also estimable by a survey, although individuals may be
unwilling or unable to disclose how many contacts they have had. The estimation
of \gls{alpha} is more complex, as this parameter is most strongly reflected in
the connectivity of very high degree nodes, who are rare in the population.
Locating them might require contact tracing, or respondent-driven
...
beter quantify the identifiability. Furthermore, doing marginal estimation
...
a larger number of pakticles, more simulated datasets per particle, and a
...
not explicity consider the trees' topologies, but it does use their normalized
...
parameters except \gls{m}}, 50\% and 95\% highest denstity intervals wer
...
parameters are more identifiable. Two retrospective experiments were peformed
...
the posterior mean estimates, we analysed the \gls{alpha} and \gls{I}
...
<<treestats, include=FALSE>>=
    ...
    alpha.ratio.cor <- alpha.stats[,cor.test(int.tip.ratio, alpha, method="spearman")]
    I.ratio.cor <- I.stats[,cor.test(int.tip.ratio, I, method="spearman")]
    m.ratio.cor <- m.stats[,cor.test(int.tip.ratio, m, method="spearman")]
    N.ratio.cor <- N.stats[,cor.test(int.tip.ratio, N, method="spearman")]
...
The accuracy of each classifier \add{on all \gls{BA} parameters is shown in}
\del{varied based on the parameter being tested} \cref{fig:rsquared}.
Classifiers based on \del{two other tree statistics,} the \gls{nltt} and
...
tips represented. However, \gls{m}, both the number and proportion of sampled
tips had a strong impact or the accuracy of the \gls{kSVR}
(\cref{fig:mcrossv}).}
...
\begin{figure}[ht]
    ...
    \caption[
        ...
        cross-validations. \add{The classifiers for \gls{I} were not evaluated
        with 1000-tip trees, because one of the tested \gls{I} values was 500,
...
The accuracy of classifiers for $I$ \del{varied significantly
...
<<gridsearch, include=FALSE>>=
    ...
    tests <- d[,list(rsquared=cor.test(true_value, point.est)$estimate^2,
                     grid.err=mean(abs(true_value - point.est) * 100 / diff(range(true_value))),
                     abs.err=mean(abs(true_value - point.est)),
                     rel.err=mean(abs(true_value - point.est) * 100 / true_value)),
                by=parameter]
...
\gls{kSVR} classifiers.} \Cref{fig:gridest} shows point estimates and
...
representing
    $\Sexpr{tests["alpha", round(grid.err)]}$\%,
    $\Sexpr{tests["I", round(grid.err)]}$\%,
    $\Sexpr{tests["m", round(grid.err)]}$\%, and
    $\Sexpr{tests["N", round(grid.err)]}$\%
of the respective grids. For \gls{I} and \gls{N}, relative errors may be more
appropriate to consider. An overestimate of 100 individuals would be very
misleading if the true population was only of size 100, but almost negligable
in a population of size 5000. The mean relative errors for \gls{I} and \gls{N}
respectively were
    $\Sexpr{tests["I", round(rel.err)]}$\% and 
...
\leq 0.5$ (on average 
    \Sexpr{d[parameter == "alpha" & true_value <= 0.5, round(mean(lower.50), 2)]} -
    \Sexpr{d[parameter == "alpha" & true_value <= 0.5, round(mean(upper.50), 2)]})
and for $\gls{alpha} \geq 1.5$ 
    (\Sexpr{d[parameter == "alpha" & true_value >= 1.5, round(mean(lower.50), 2)]} -
    \Sexpr{d[parameter == "alpha" & true_value >= 1.5, round(mean(upper.50), 2)]}).
For \gls{I}, similar 50\% \gls{HPD} were observed for $\gls{I} \leq 1500$
(average 
    [\Sexpr{d[parameter == "I" & true_value <= 1500, round(mean(lower.50))]} -
     \Sexpr{d[parameter == "I" & true_value <= 1500, round(mean(upper.50))]}])
and for $\gls{I} > 3000$ 
    ([\Sexpr{d[parameter == "I" & true_value > 3000, round(mean(lower.50))]} -
      \Sexpr{d[parameter == "I" & true_value > 3000, round(mean(upper.50))]}]).
...
\Cref{fig:abcptm2} shows \del{MAP} \add{posterior mean} point estimates of the
\gls{BA} model parameters obtained with \gls{ABC} on simulated data. The
estimates shown correspond only to the simulations where \gls{m} was set to 2,
however the results for $\gls{m} = 3$ and $\gls{m} = 4$ were similar
...
\begin{figure}[ht]
    ...
    \caption[
        ...
        Posterior mean point estimates for \gls{BA} model parameters obtained
        by running \software{netabc} on simulated data, for simulations with
        $\gls{m} = 2$. Dashed lines indicate true values. (A) Estimates of
        ...
        fixed in these simulations at the values $\gls{m} = 2$ and 
        $\gls{N} = 5000$.
    }
    \label{fig:abcptm2}
\end{figure}
...
\begin{table*}[ht]
    ...
    \caption[
        Average posterior mean point estimates and 95\% \glspl{HDI} interval
...
for \gls{I},
    \Sexpr{d[,round(median(m_error), 2)]} 
    [\Sexpr{d[,round(quantile(m_error, 0.25), 2)]} - 
    \Sexpr{d[,round(quantile(m_error, 0.75), 2)]}]
for \gls{m}, and
    \Sexpr{d[,round(median(N_error))]} 
    [\Sexpr{d[,round(quantile(N_error, 0.25))]} - 
    \Sexpr{d[,round(quantile(N_error, 0.75))]}]
for \gls{N}. These errors comprised, respectively,
    \Sexpr{d[,round(median(alpha_error) * 100 / 2)]}\%,
    \Sexpr{d[,round(median(I_error) * 100 / 4500)]}\%,
    \Sexpr{d[,round(median(m_error) * 100 / 6)]}\%, and
    \Sexpr{d[,round(median(N_error) * 100 / 14500)]}\%
of the regions of nonzero prior density. For \gls{I} and \gls{N}, relative
errors were
    \Sexpr{d[,round(median(I_error * 100 / true_I))]}\%
    [\Sexpr{d[,round(quantile(I_error * 100 / true_I, 0.25))]} - 
    \Sexpr{d[,round(quantile(I_error * 100 / true_I, 0.75))]}\%]
and
    \Sexpr{d[,round(median(N_error * 100 / true_N))]}\%
    [\Sexpr{d[,round(quantile(N_error * 100 / true_N, 0.25))]} - 
    \Sexpr{d[,round(quantile(N_error * 100 / true_N, 0.75))]}\%].
Average \gls{HPD} interval widths were
    \Sexpr{d[,round(mean(alpha.upper.95 - alpha.lower.95), 2)]},
    \Sexpr{d[,round(mean(I.upper.95 - I.lower.95))]},
    \Sexpr{d[,round(mean(m.upper.95 - m.lower.95), 2)]}, and
    \Sexpr{d[,format(round(mean(N.upper.95 - N.lower.95)), digits=5)]},
representing 
    \Sexpr{d[,round(mean(alpha.upper.95 - alpha.lower.95) * 100 / 2)]}\%,
    \Sexpr{d[,round(mean(I.upper.95 - I.lower.95) * 100 / 4500)]}\%,
    \Sexpr{d[,round(mean(m.upper.95 - m.lower.95) * 100 / 6)]}\%, and
    \Sexpr{d[,round(mean(N.upper.95 - N.lower.95) * 100 / 14500)]}\%
of the nonzero prior density regions. Point estimates of \gls{I} were upwardly
biased: \gls{I} was overestimated in
    \Sexpr{d[,sum(I > true_I)]}
out of 
    \Sexpr{nrow(d)}
simulations
    (\Sexpr{round(d[,sum(I > true_I)] * 100 / nrow(d))}\%).
The estimates for \gls{m} and \gls{N} were similar across all simulations
(median [IQR] point estimates
    \Sexpr{d[,round(median(m), 2)]}
    [\Sexpr{d[,round(quantile(m, 0.25), 2)]} - 
    \Sexpr{d[,round(quantile(m, 0.75), 2)]}]
and 
    \Sexpr{d[,round(median(N))]}
    [\Sexpr{d[,round(quantile(N, 0.25))]} - 
    \Sexpr{d[,round(quantile(N, 0.75))]}])
regardless of the true values of any of the \gls{BA} parameters.}

\add{To analyse the effects of the true parameter values on the
...
(\cref{fig:abcptm2,fig:abcptm3,fig:abcptm4}), \glspl{GLM} were not fitted for
these parameters. The estimated coefficients are shown in
\cref{tab:glmalpha,tab:glmI}. For the parameters \gls{alpha}, \gls{I}, and
\gls{N}, the \gls{GLM} were fitted using the inverse link function. That is, if
$p$ is the true value of the parameter and $\hat{p}$ is a random variable
representing our estimate of the parameter, the \gls{GLM} posits a relationship
of the form}
\[
...
<<abc_glm, include=FALSE>>=
    stopifnot(alpha.glm[Parameter == "alpha",p] < 0.05)
    stopifnot(alpha.glm[Parameter == "I",p] > 0.05)
    stopifnot(alpha.glm[Parameter == "m",p] < 0.05)

    stopifnot(I.glm[Parameter == "alpha",p] < 0.05)
    stopifnot(I.glm[Parameter == "I",p] < 0.05)
    stopifnot(I.glm[Parameter == "m",p] > 0.05)
@
...
<<glm_tables, echo=FALSE, warning=FALSE, error=FALSE, results="asis">>=
    ...
    format.glm <- function (df) {
        df.copy <- copy(df)
        df.copy[Parameter == "alpha", Parameter := "$\\alpha$"]
        df.copy[,p := paste0("$", sapply(p, pp, eq=FALSE), "$")]
        df.copy[,Estimate := paste0("$", sapply(Estimate, pp, eq=FALSE, thres=NA), "$")]
        df.copy[,`Standard error` := paste0("$", sapply(`Standard error`, pp, eq=FALSE, thres=NA), "$")]
        setnames(df.copy, "p", "$p$-value")
        df.copy
    }

    alpha.glm.tbl <- format.glm(alpha.glm)
    I.glm.tbl <- format.glm(I.glm)

    capt.short <- paste("Parameters of a fitted \\gls{GLM} relating error in estimated",
                        "\\gls{alpha} to true values of \\gls{BA} parameters.")
    capt <- paste(capt.short, "\\gls{GLM} was fitted with a Gaussian distribution and",
                  "inverse link function. Coefficients are interpretable as",
                  "additive effects on the inverse of the mean error.")
    print(xtable(alpha.glm.tbl, label="tab:glmalpha", caption=c(capt, capt.short)),
          include.rownames=FALSE, sanitize.text.function=identity,
          sanitize.colnames.function=identity)

    capt.short <- paste("Parameters of a fitted \\gls{GLM} relating error in estimated",
                        "\\gls{I} to true values of \\gls{BA} parameters.")
    capt <- paste(capt.short, "GLM was fitted with a Gaussian distribution and",
                  "inverse link function. Coefficients are interpretable as",
                  "additive effects on the inverse of the mean error.")
    print(xtable(I.glm.tbl, label="tab:glmI", caption=c(capt, capt.short)),
          include.rownames=FALSE, sanitize.text.function=identity,
          sanitize.colnames.function=identity)
@
...
\gls{alpha} decreased with larger true values of \gls{alpha} 
    ($p \Sexpr{pp(alpha.glm[Parameter == "alpha", p], eq=TRUE)}$)
and \gls{m}
    ($p \Sexpr{pp(alpha.glm[Parameter == "m", p], eq=TRUE)}$)
...
values of 0 and 0.5 (\cref{fig:abcptm2}). The error in the estimated prevalence
\gls{I} was slightly lower for smaller values of \gls{alpha}
    ($p \Sexpr{pp(I.glm[Parameter == "alpha", p], eq=TRUE)}$)
and \gls{I}
    ($p \Sexpr{pp(I.glm[Parameter == "I", p], eq=TRUE)}$),
...
results from the \gls{kSVR} classifier. Of the four parameters, $\alpha$ was
...
than those for the other values 
    ...
    $\Sexpr{latexSN(round(zero.alpha.test[["p.value"]], 4))}$).
...
at least 1
    (Wilcoxon rank-sum test, $p$ = 
    $\Sexpr{latexSN(round(alpha.I.test[["p.value"]], 4))}$)
...
<<supp_labels, echo=FALSE, results="asis">>=
    ...
    worst <- d[alpha_error > quantile(alpha_error, 0.75) & I_error > quantile(I_error, 0.75),]
    best <- d[alpha_error < quantile(alpha_error, 0.25) & I_error < quantile(I_error, 0.25),]
    stopifnot(nrow(worst) == 1)
    stopifnot(nrow(best) == 1)

    worst.plot <- worst[,sprintf("%.1f_%.0f_%.0f_%.0f_%.0f", true_alpha, true_I, true_m, true_N, replicate)]
    best.plot <- best[,sprintf("%.1f_%.0f_%.0f_%.0f_%.0f", true_alpha, true_I, true_m, true_N, replicate)]
@
...
simulation with relatively low error, where the errors in \gls{alpha} and
\gls{I} were each below the 25\% quantiles. The parameters for this simulation
were 
    $\gls{alpha} = \Sexpr{best[,true_alpha]}$,
    $\gls{I} = \Sexpr{best[,true_I]}$,
    $\gls{m} = \Sexpr{best[,true_m]}$, and
    $\gls{N} = \Sexpr{best[,true_N]}$.
\Cref{fig:abchigh1,abchigh2} show the equivalent marginals for a different
simulation with relatively high error rates above the 75\% quantiles. The
parameters were
    $\gls{alpha} = \Sexpr{worst[,true_alpha]}$,
    $\gls{I} = \Sexpr{worst[,true_I]}$,
    $\gls{m} = \Sexpr{worst[,true_m]}$, and
    $\gls{N} = \Sexpr{worst[,true_N]}$.
The two-dimensional marginals some dependence between pairs of parameters,
particularly \gls{I} and \gls{N} which show a diagonally shaped region of high
posterior density.} 

\begin{figure}[ht]
    \centering
    \includegraphics[width=\textwidth]{{\Sexpr{file.path("abc-posterior", best.plot)}}.pdf}
    \caption[
...
\begin{figure}[ht]
    \centering
    \includegraphics[width=\textwidth]{{\Sexpr{file.path("abc-posterior-2d", best.plot)}}.pdf}
    \caption[
...
\begin{figure}[ht]
    \centering
    \includegraphics[width=\textwidth]{{\Sexpr{file.path("abc-posterior", worst.plot)}}.pdf}
    \caption[
        One-dimensional marginal posterior distributions of \gls{BA} model
...
\begin{figure}[ht]
    \centering
    \includegraphics[width=\textwidth]{{\Sexpr{file.path("abc-posterior-2d", worst.plot)}}.pdf}
    \caption[
        Two-dimensional marginal posterior distributions of \gls{BA} model
        parameters estimated with high error by \software{netabc} from a
        simulated transmission tree.
...
<<mixed_peerdriven, include=FALSE>>=
  ...
  f <- function (x, wt) {
    hpd <- wtd.hpd(x, wt/sum(wt))
    list(pmean=wtd.mean(x, wt, normwt=TRUE), 
         hpd.min=hpd[1], 
         hpd.max=hpd[2])
  }
  d <- d[,f(value, weight), by=parameter]
  setkey(d, parameter)
  ...
  p <- p[,f(value, weight), by=parameter]
  setkey(p, parameter)
  options(scipen=5)
@
...
\add{\Cref{fig:marginal} shows the effect of performing marginal \gls{ABC}
estimation of each of the \gls{BA} parameters on the same simulated
transmission tree. The estimates of \gls{m} are apparently unaffected by
marginalizing out the other parameters, coroborating the prevous experiments'
findings that \gls{m} is not an identifiable parameter from scaled tree shapes.
The estimates of \gls{alpha} and \gls{N} were weakly improved in marginal
estimation, which is consistent with the result from grid search that all
\gls{N} values and \gls{alpha} values below 0.5 are weakly identifiable. On the
other hand, both the accuracy and precision (\ie width of the \gls{HPD}
intervals) of the estimates of \gls{I} were substantially improved when \gls{N}
was marginalized out. Neither the accuracy nor the precision of any parameter
estimates appeared to improve when more computing power was used for \gls{SMC}
(\cref{fig:moresmc}).}
...
\begin{figure}[ht]
    ...
    \caption[
        Comparsion of \gls{BA} parameter estimates obtained with
...
\cref{tab:data}. In addition \add{to the published data}, we analysed an
...
subtype B sequences and labelled this dataset as \gls{MSM}. Three datasets
(IDU/Estonia, HET/Uganda, and HET/Malawi) included both \textit{env} and
\textit{gag} sequences. Each gene was analyzed separately to assess the
robustness of \software{netabc} to the particular \gls{HIV} gene sequence used
to estimate a transmission tree.}
...
\del{Three} \add{Six} of the datasets (MSM/Shanghai, HET/Botswana, HET/Uganda, 
...
chosen by eye such that the individuals were monophyletic in both the
...
<<realdata, include=FALSE>>=
  ...
  d1 <- fread(rownames(m)[m$m_min == 1 & m$agg_gene == "gag"])
  d2 <- fread(rownames(m)[m$m_min == 2 & m$agg_gene == "gag"])

  d1.env <- fread(rownames(m)[m$m_min == 1 & m$agg_gene == "env"])
  d2.env <- fread(rownames(m)[m$m_min == 2 & m$agg_gene == "env"])

  pmean1 <- dcast.data.table(d1, dataset~parameter, value.var="mean")
  up1 <- dcast.data.table(d1, dataset~parameter, value.var="hpd.max.95")
  low1 <- dcast.data.table(d1, dataset~parameter, value.var="hpd.min.95")
  setkey(pmean1, dataset)
  setkey(up1, dataset)
  setkey(low1, dataset)

  pmean2 <- dcast.data.table(d2, dataset~parameter, value.var="mean")
  up2 <- dcast.data.table(d2, dataset~parameter, value.var="hpd.max.95")
  low2 <- dcast.data.table(d2, dataset~parameter, value.var="hpd.min.95")
  setkey(pmean2, dataset)
  setkey(up2, dataset)
  setkey(low2, dataset)

  pmean1.env <- dcast.data.table(d1.env, dataset~parameter, value.var="mean")
  up1.env <- dcast.data.table(d1.env, dataset~parameter, value.var="hpd.max.95")
  low1.env <- dcast.data.table(d1.env, dataset~parameter, value.var="hpd.min.95")
  setkey(pmean1.env, dataset)
  setkey(up1.env, dataset)
  setkey(low1.env, dataset)

  pmean2.env <- dcast.data.table(d2.env, dataset~parameter, value.var="mean")
  up2.env <- dcast.data.table(d2.env, dataset~parameter, value.var="hpd.max.95")
  low2.env <- dcast.data.table(d2.env, dataset~parameter, value.var="hpd.min.95")
  setkey(pmean2.env, dataset)
  setkey(up2.env, dataset)
  setkey(low2.env, dataset)

  setkey(d1, dataset)
  setkey(d2, dataset)
  setkey(d1.env, dataset)
  setkey(d2.env, dataset)

  pmean1 <- unique(d1)[,.(dataset, ntip)][pmean1]
  pmean2 <- unique(d2)[,.(dataset, ntip)][pmean2]
  pmean1.env <- unique(d1.env)[,.(dataset, ntip)][pmean1.env]
  pmean2.env <- unique(d2.env)[,.(dataset, ntip)][pmean2.env]

  stopifnot(pmean1[,dataset[which.max(alpha)]] == "zetterberg2004")
  stopifnot(pmean1[,dataset[which.min(alpha)]] == "cuevas2009")

  stopifnot(pmean1[,dataset[which.min(I)]] == "zetterberg2004")
  stopifnot(pmean1[,dataset[which.max(I)]] == "novitsky2014")
  stopifnot(pmean1[dataset != "novitsky2014", dataset[which.max(I)]] == "bctree")

  stopifnot(pmean1[,cor.test(ntip, I, method="spearman")$p.value] > 0.05)
@
...
estimates.} \add{Posterior mean point estimates and 50\% and 95\% \glspl{HPD}
for each parameter are shown in \cref{fig:abchpd}.} \del{and marginal posterior
distributions in \cref{fig:cuevas,fig:li,fig:niculescu,fig:novitsky,fig:wang}}.
\add{\Cref{fig:abchpdm2} shows point estimates and \glspl{HPD} obtained when
...
\begin{figure}[ht]
    ...
    \caption[
        ...
        (lines) \gls{HPD} intervals for parameters of the \gls{BA} network
        ...
        heterosexual. Note that posterior means can fall outside of \glspl{HPD} 
        if the distribution is diffuse.
    }
    \label{fig:abchpd}
\end{figure}
...
mean [95\% \gls{HPD}] 
  \Sexpr{pmean1["niculescu2015", round(alpha, 2)]} 
  [\Sexpr{low1["niculescu2015", round(alpha, 2)]} - 
   \Sexpr{up1["niculescu2015", round(alpha, 2)]}]
and
  \Sexpr{pmean1["wang2015", round(alpha, 2)]} 
...
low estimated $\alpha$ values
  (\Sexpr{pmean1["li2015", round(alpha, 2)]} 
  [\Sexpr{low1["li2015", round(alpha, 2)]} - 
  \Sexpr{up1["li2015", round(alpha, 2)]}]
and
  \Sexpr{pmean1["cuevas2009", round(alpha, 2)]} 
...
HPD intervals remained roughly the same
  (\Sexpr{pmean2["li2015", round(alpha, 2)]} 
  [\Sexpr{low2["li2015", round(alpha, 2)]} - 
  \Sexpr{up2["li2015", round(alpha, 2)]}]
and
  \Sexpr{pmean2["cuevas2009", round(alpha, 2)]} 
...
for both priors on $m$
  (\Sexpr{pmean1["novitsky2014", round(alpha, 2)]} for $m \geq 1$;
   \Sexpr{pmean2["novitsky2014", round(alpha, 2)]} for $m \geq 2$).
...
were all sub-linear, ranging from 
    \Sexpr{pmean1["zetterberg2004", round(alpha, 2)]}
for the \gls{IDU}/Estonia data to
    \Sexpr{pmean1["cuevas2009", round(alpha, 2)]}
for the mixed/Spain data. When aggregated by risk group, the average estimates
were
    \Sexpr{pmean1[c("zetterberg2004", "niculescu2015", "bctree"), round(mean(alpha), 2)]}
for \gls{IDU},
    \Sexpr{pmean1[c("novitsky2014", "mccormack2002", "grabowski2014"), round(mean(alpha), 2)]}
for primarily heterosexual risk, and
    \Sexpr{pmean1[c("wang2015", "kao2011", "little2014", "li2015"), round(mean(alpha), 2)]}
...
\gls{HPD} were very wide for most datasets, often encompassing almost the
entire range from 0 to 1 (\cref{fig:abchpd}). For all the datasets except
HET/Malawi and MSM/Beijing, the posterior mean was either outside, or very
close to the border of, the 50\% \gls{HPD}. This indicates that the posterior
distributions were diffuse with heavy tails, rather than having most of their
mass around the mode. When the value $\gls{m} = 1$ was disallowed by the prior, 
the separation between the \gls{IDU} datasets and the others became more
striking (\cref{fig:abchpdm2}). All three \gls{IDU} datasets had estimated
\gls{alpha} values at or above
    \Sexpr{pmean2[c("zetterberg2004", "niculescu2015", "bctree"), round(min(alpha), 2)]}.
The estimate for the MSM/Beijing data was slightly lower
    (\Sexpr{pmean2["wang2015", round(alpha, 2)]}) and
the estimates for the seven remaining non-\gls{IDU} datasets were bounded above by
    \Sexpr{pmean2[c("novitsky2014", "mccormack2002", "grabowski2014", "kao2011", "little2014", "li2015", "cuevas2009"), round(max(alpha), 2)]}.}
...
\begin{figure}
    ...
    \caption[
        Posterior means and 50\%/95\% \gls{HPD} intervals for parameters of the
        \gls{BA} network model, fitted to three \gls{HIV} datasets where both
        \textit{gag} and \textit{env} genes were sequenced.
    ]{
        Posterior means (points) 50\% \gls{HPD} intervals (notches), and 95\%
        \gls{HPD} intervals (lines) for parameters of the \gls{BA} network model,
        fitted to three \gls{HIV} datasets where both \textit{gag} and
...
relatively narrow HPD intervals compared to the nonzero prior density region
  (\citeauthor{cuevas2009hiv}, \Sexpr{pmean1["cuevas2009", floor(I)]} 
  [\Sexpr{low1["cuevas2009", floor(I)]} -
   \Sexpr{up1["cuevas2009", floor(I)]}];
   \citeauthor{niculescu2015recent}, \Sexpr{pmean1["niculescu2015", floor(I)]}
  [\Sexpr{low1["niculescu2015", floor(I)]} - 
   \Sexpr{up1["niculescu2015", floor(I)]}];
  \citeauthor{li2015hiv}, \Sexpr{pmean1["li2015", floor(I)]} 
  [\Sexpr{low1["li2015", floor(I)]} -
   \Sexpr{up1["li2015", floor(I)]}];
   \citeauthor{wang2015targeting}, \Sexpr{pmean1["wang2015", floor(I)]}
...
estimated $I$, and HPD interval spanning almost the entire prior region
  (\Sexpr{pmean1["novitsky2014", floor(I)]} 
...
<<genetrees, include=FALSE>>=
    datasets <- c("grabowski2014", "zetterberg2004", "mccormack2002")
    paths <- paste0("../../simulations/", datasets, "/subtree/*")
    paths <- sapply(paths, Sys.glob)
    trees <- lapply(paths, read.tree)
    get.ratio <- function (t) {
        int <- t$edge.length[t$edge[,2] > Ntip(t)]
        tip <- t$edge.length[t$edge[,2] <= Ntip(t)]
        mean(int) / mean(tip)
    }
    ratios <- data.table(dataset=rep(datasets, each=2),
                         gene=rep(c("env", "gag"), 3),
                         ratio=sapply(trees, get.ratio))
    stopifnot(ratios[gene == "env", mean(ratio)] <
              ratios[gene == "gag", mean(ratio)])
@

\add{For all but the HET/Botswana data, the posterior mean estimates for
the prevalence \gls{I} were between
    \Sexpr{pmean1["zetterberg2004", round(I)]}
(IDU/Estonia) and
    \Sexpr{pmean1["bctree", round(I)]}
(IDU/Canada). The HET/Botswana data had a much higher estimated \gls{I} value
    (\Sexpr{pmean1["novitsky2014", round(I)]})
...
prevalence (Spearman correlation, 
    $p \Sexpr{pp(pmean1[,cor.test(ntip, I, method="spearman")[["p.value"]]], eq=TRUE)}$),
...
were analyzed, the estimates from the \textit{env} data were substantially
higher (\cref{fig:genes}). The ratios of internal to terminal branch lengths
were lower for the \textit{env} trees than the \textit{gag} trees (average 
    \Sexpr{ratios[gene == "env", round(mean(ratio), 2)]}
for \textit{env} and 
    \Sexpr{ratios[gene == "gag", round(mean(ratio), 2)]}
for \textit{gag}). As with \gls{alpha}, many of the posterior means were
outside the 50\% \gls{HPD} intervals, suggesting diffuse posterior
distributions. The values of \gls{I} were fairly robust to restricting the
prior to $\gls{m} \geq 2$ (compare \cref{fig:abchpd,fig:abchdpm2}), although 
the 95\% \gls{HPD} intervals were slightly narrower (average width
    \Sexpr{round(mean(up1[,I] - low1[,I]))}
for $\gls{m} \geq 1$ and
    \Sexpr{round(mean(up2[,I] - low2[,I]))}
for $\gls{m} \geq 2$).}
...
the \citeauthor{li2015hiv} data, for which the estimate was lower 
  (\Sexpr{pmean1["li2015", floor(N)]})
when $m = 1$ was allowed.}

\add{The posterior means of \gls{m} were equal to one for 
  \Sexpr{replace_number(pmean1[,sum(m == 1)])}
of the datasets analysed. The widths of the 95\% \gls{HPD} intervals varied
from 0 (all the mass on the estimated value) to 5 (the entire prior region).
When the value $\gls{m} = 1$ was disallowed by the prior, the posterior means
for all but the HET/Botswana data took on the value 3, with the \gls{HPD}
intervals spanning the entire prior region. This is very similar to the results 
observed for \gls{m} on simulated data (\cref{fig:abcptm2,tab:abchpd}), and
suggests that \gls{m} is not identifiable from these data with this prior. On
the other hand, the estimate and 95\% \gls{HPD} interval for the HET/Botswana
data (4 [2-5]) was the same for both priors.}

\add{Estimates of \gls{N} were  mostly uninformative, with very similar estimates
for all datasets (mean
    \Sexpr{pmean1[,round(mean(N))]},
range
    \Sexpr{pmean1[,round(min(N))]} - \Sexpr{pmean1[,round(max(N))]}).
...
allowed by the prior. The results did not change appreciably when the value
$\gls{m} = 1$ was assigned zero prior probability.}
