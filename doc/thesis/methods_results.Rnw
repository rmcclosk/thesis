...
<<treestats, include=FALSE>>=
    ...
    get.treestats <- function (param) {
        ...
        rownames(m) <- sub(".bk.2", "", rownames(m))
...
epidemic scenario, with average accuracy values of 
  \Sexpr{kernel.m[,round(mean(accuracy), 2)]} for \gls{kSVM},
  \Sexpr{nltt.m[,round(mean(accuracy), 2)]} for the \gls{nltt}, and
...
on 1000-tip trees 
    (average 
     \Sexpr{kernel.m[ntip == 100,round(mean(accuracy), 2)]},
     \Sexpr{kernel.m[ntip == 500,round(mean(accuracy), 2)]},
     \Sexpr{kernel.m[ntip == 1000,round(mean(accuracy), 2)]}
...
\begin{figure}[ht]
    \centering
    \includegraphics[width=0.8\textwidth]{{\Sexpr{file.path("abc-posterior", best.plot)}}.pdf}
    \caption[
...
\begin{figure}[ht]
    \centering
    \includegraphics[width=\textwidth]{{\Sexpr{file.path("abc-posterior-2d", best.plot)}}.png}
    \caption[
...
\begin{figure}[ht]
    \centering
    \includegraphics[width=0.8\textwidth]{{\Sexpr{file.path("abc-posterior", worst.plot)}}.pdf}
    \caption[
        One-dimensional marginal posterior distributions of \gls{BA} model
...
\begin{figure}[ht]
    \centering
    \includegraphics[width=\textwidth]{{\Sexpr{file.path("abc-posterior-2d", worst.plot)}}.png}
    \caption[
        Two-dimensional marginal posterior distributions of \gls{BA} model
        parameters estimated with high error by \software{netabc} from a
        simulated transmission tree.
