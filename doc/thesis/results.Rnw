...
of the four \gls{BA} model parameters, we simulated trees under three distinct
values of that parameter while holding the others fixed, then cross-validated a
kernel-\gls{SVM} classifier for that parameter.

<<treestats, include=FALSE>>=
    #params <- c("alpha", "I", "m", "N")
    #ptn <- paste0("../../simulations/kernel-", params, "/statistics/*")
    #files <- lapply(ptn, Sys.glob)
    #d <- lapply(files, collect.data)
    #d <- mapply(cbind, d, parameter=as.list(params), SIMPLIFY=FALSE)
    #lapply(d, setDT)
    #d[[1]] <- subset(d[[1]], m == 2)
    #setnames(d[[1]], "nsimnode", "I")
@

% trees look different
Trees simulated under different values of \gls{alpha} are visibly quite
distinctive (\cref{fig:patrees}). In particular, higher values of \gls{alpha}
...
compared to networks with lower \gls{alpha} values. None of the other three
parameters produced trees which were as easily distinguished from each other
(\cref{fig:Itrees,fig:mtrees,fig:Ntrees,fig:Itrees}). However, higher $I$
values produce trees which appear to have longer terminal branches, 
