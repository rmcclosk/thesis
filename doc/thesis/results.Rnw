...
\gls{alpha} and \gls{I}, and positively corelated with \gls{N} and \gls{m}
  ...
  $p$-values
    $\Sexpr{latexSN(round(alpha.ratio.cor$p.value, -floor(log10(alpha.ratio.cor$p.value))))}$,
    $\Sexpr{latexSN(round(I.ratio.cor$p.value, -floor(log10(I.ratio.cor$p.value))))}$,
    $\Sexpr{latexSN(round(m.ratio.cor$p.value, -floor(log10(m.ratio.cor$p.value))))}$,
    $\Sexpr{latexSN(round(N.ratio.cor$p.value, -floor(log10(N.ratio.cor$p.value))))}$).
...
$p$-value = 
    $\Sexpr{tip.cor["alpha", latexSN(round(p.value, -floor(log10(p.value))))]}$,
    $\Sexpr{tip.cor["I", latexSN(round(p.value, -floor(log10(p.value))))]}$,
    $\Sexpr{tip.cor["N", latexSN(round(p.value, -floor(log10(p.value))))]})$.
...
with the true value of \gls{alpha} 
    (Spearman's $\rho$ = \Sexpr{value.cor["alpha", round(estimate, 2)]},
     $p = \Sexpr{value.cor["alpha", latexSN(round(p.value, -floor(log10(p.value))))]}$),
...
\gls{m}
    (Spearman's $\rho$ = \Sexpr{value.cor["m", round(estimate, 2)]},
     $p = \Sexpr{value.cor["m", latexSN(round(p.value, -floor(log10(p.value))))]}$).
This positive correlation was apparently due to the much lower error for $m =
1$ than for the other $m$ values (\cref{fig:gridm})
    (mean errors \Sexpr{d[parameter == "m" & true_value == 1, mean(error)]} for
     $m = 1$ vs. \Sexpr{d[parameter == "m" & true_value > 1, mean(error)]} for
     $m > 1$).
...
of \gls{N}
    (Spearman's $\rho$ = \Sexpr{value.cor["N", round(estimate, 2)]},
     $p = \Sexpr{value.cor["N", latexSN(round(p.value, -floor(log10(p.value))))]}$).
